# Using Buckit with Veeam

When using Veeam Backup and Replication, you can use S3 compatible object storage such as Buckit as a capacity tier for backups.  This disaggregates storage for the Veeam infrastructure and allows you to retain control of your data. With the ease of use of setup and administration of Buckit, it allows a Veeam backup admin to easily deploy their own object store for capacity tiering.
...
- Buckit object storage set up per <https://docs.min.io/community/minio-object-store/index.html#procedure>
...
- Configure the Buckit Client for the Veeam Buckit endpoint - <https://docs.min.io/community/minio-object-store/reference/bm-cli.html>
...
```
bm mb mybuckit/veeambackup
```

> NOTE: For Veeam Backup with Immutability, create the bucket with object lock enabled, e.g.,

```
bm mb -l mybuckit/veeambackup
```

> Object locking requires erasure coding enabled on the Buckit server. For more information see <https://docs.min.io/community/minio-object-store/operations/concepts/erasure-coding.html>.

### Add Buckit as an object store for Veeam

Follow the steps from the Veeam documentation for adding Buckit as an object store - <https://helpcenter.veeam.com/docs/backup/vsphere/adding_s3c_object_storage.html?ver=100>
...
- Follow the wizard.  Under the "Object Storage Backup Repository" section, choose the Buckit object storage you created above
...
The next time the backup job runs, you can use the  `mc admin trace mybuckit` command and verify traffic is flowing to the Buckit nodes. For Veeam Backup and Replication you will need to wait for the backup to complete to the performance tier before it migrates data to the capacity tier (i.e., Buckit).
