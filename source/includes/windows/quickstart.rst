...
.. note::
   ...
   Due to NTFS behaviors and limitations, Buckit does not recommend multi-node multi-drive deployments on Windows hosts.

   Use :minio-docs:`Linux hosts <minio/linux/operations/installation.html>` or :minio-docs:`Kubernetes <minio/kubernetes/upstream/index.html>` for deploying production-ready distributed Buckit deployments.

Use Windows-based Buckit deployments for early development and evaluation.
Buckit strongly recommends Linux (RHEL, Ubuntu) systems for long-term development and production environments.

Buckit supports non-EOL Windows versions (Windows 10, Windows Server 2016+). 
...
#. Install the Buckit Server
   ...
   Download the Buckit executable from the following URL:
...
#. Launch the :mc:`minio server`
   ...
   Use this command to start a local Buckit instance in the ``C:\minio`` folder.
