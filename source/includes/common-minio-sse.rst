...
Buckit generates a Data Encryption Key (DEK) using the |EK|. 
Specifically, :minio-git:`Buckit Key Encryption Service (KES) <kes>` requests a new cryptographic key from the KMS using the |EK| as the "root" key. 

KES returns both the plain-text *and* an |EK|-encrypted representation of the DEK. 
Buckit stores the encrypted representation as part of the object metadata.

.. end-sse-dek

.. start-sse-kek

Buckit uses a deterministic algorithm to generate a 256-bit unique Key Encryption Key (KEK). 
...
Buckit generates the KEK at the time of each cryptographic encryption or decryption operation and *never* stores the KEK to a drive.

.. end-sse-kek

.. start-sse-oek

Buckit generates a random 256-bit unique Object Encryption Key (OEK) and uses that key to encrypt the object. 
Buckit never stores the plaintext representation of the OEK on a drive. 
...
.. mc-cmd:: --enc-c
   ...
   .. note::
      ...
      Buckit strongly recommends against using SSE-C encryption in production workloads.
