...
This step creates users and policies on the Buckit deployment for supporting
...
on the deployment. See :ref:`minio-users` and :ref:`Buckit Policy Based Access Control <minio-policy>` for more
complete documentation on Buckit users and policies respectively.

.. end-create-transition-user-desc

.. start-transition-bucket-access-desc

Buckit *requires* exclusive access to the transitioned data on the remote storage tier.
Object metadata on the "hot" Buckit source is strongly linked to the object data on the "warm/cold" remote tier.
Buckit cannot retrieve object data without access to the remote, nor can the remote be used to restore lost metadata on the source.
...
Buckit ignores any objects in the remote bucket or bucket prefix not explicitly managed by the Buckit deployment. 
...
Buckit stores all transitioned objects in the remote storage bucket or resource under a unique per-deployment prefix value. 
This value is not intended to support identifying the source deployment from the backend. 
Buckit supports an additional optional human-readable prefix when configuring the remote target, which may facilitate operations related to diagnostics, maintenance, or disaster recovery. 

Buckit recommends specifying this optional prefix for remote storage tiers which contain other data, including transitioned objects from other Buckit deployments.
...
Buckit creates metadata for each transitioned object that identifies its location on the remote storage. 
Applications cannot trivially identify and access a transitioned object independent of Buckit.
...
deployment topologies provide for all objects on the Buckit deployment. Using
...
protections should implement Buckit :ref:`Server-Side replication
