...
The following procedure installs the latest stable version (|operator-version-stable|) of the Buckit Operator on Kubernetes infrastructure.

The Buckit Operator installs a :kube-docs:`Custom Resource Definition (CRD) <concepts/extend-kubernetes/api-extension/custom-resources/#customresourcedefinitions>` to support describing Buckit tenants as a Kubernetes :kube-docs:`object <concepts/overview/working-with-objects/kubernetes-objects/>`. 
See the Buckit Operator :minio-git:`CRD Reference <operator/blob/master/docs/tenant_crd.adoc>` for complete documentation on the Buckit CRD.
...
The Operator pods monitor all namespaces by default for objects using the Buckit CRD and manages those resources automatically.
...
- Buckit Container that runs all of the standard Buckit functions, equivalent to basic Buckit installation on baremetal.
...
- Sidecar container used to initialize the Buckit tenant.
  ...
  .. versionchanged:: Operator 6.0.0
     ...
     The Sidecar has its own image and release cycle separate from the rest of the Buckit Operator.
     The Buckit Operator stores the tenant's environment variables in the sidecar, allowing the Operator to update the variables without requiring a rolling restart.
...
..    :width: 600px
..    :alt: A diagram of the namespaces and pods used by or maintained by the Buckit Operator.
..    :align: center
...
Buckit tests |operator-version-stable| against a floor of Kubernetes API of |k8s-floor|.
Buckit **strongly recommends** maintaining Kubernetes infrastructure using `actively maintained Kubernetes API versions <https://kubernetes.io/releases/>`__.


Buckit **strongly recommends** upgrading Kubernetes clusters running with `End-Of-Life API versions <https://kubernetes.io/releases/patch-releases/#non-active-branch-history>`__
...
The `default Buckit Operator Kustomize template <https://github.com/minio/operator/blob/master/kustomization.yaml>`__ provides a starting point for customizing configurations for your local environment.
...
Buckit recommends ECDSA (e.g. `NIST P-256 curve <https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf>`__) or EdDSA (e.g. :rfc:`Curve25519 <7748>`) TLS private keys/certificates due to their lower computation requirements compared to RSA.
...
.. important::
   ...
   The Buckit Operator automatically generates TLS certificates for all Buckit Tenant pods using the specified Certificate Authority (CA).
   Clients external to the Kubernetes cluster must trust the Kubernetes cluster CA to connect to the Buckit Operator or Buckit Tenants. 

   Clients which cannot trust the Kubernetes cluster CA can disable TLS validation for connections to the Buckit Operator or a Buckit Tenant. 

   Alternatively, you can generate x.509 TLS certificates signed by a known and trusted CA and pass those certificates to Buckit Tenants. 
   See :ref:`minio-tls` for more complete documentation.

Certificate Management with cert-manager
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Rather than the Buckit Operator managing certificates, you can configure the deployment to use `cert-manager <https://cert-manager.io/>`__.
For instructions for deploying the Buckit Operator and tenants using cert-manager, refer to the :ref:`cert-manager page <minio-certmanager>`.

Procedure
---------

The following steps deploy Operator using Kustomize and a ``kustomization.yaml`` file from the Buckit Operator GitHub repository.
