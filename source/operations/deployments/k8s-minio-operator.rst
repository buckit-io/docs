...
The Buckit Operator exists in its own namespace.
Within the Operator's namespace, the Buckit Operator utilizes two pods:
...
Buckit supports `maintained Kubernetes APIs <https://kubernetes.io/releases/>`__ for deploying the Operator.
...
- For the Buckit Console when the :envvar:`MINIO_CONSOLE_TLS_ENABLE` environment variable is set to ``on``.
...
The Buckit Operator reads certificates inside the ``operator-ca-tls`` secret and syncs this secret within the tenant namespace to trust private certificate authorities, such as when using cert-manager.
