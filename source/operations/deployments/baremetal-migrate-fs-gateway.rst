...
.. note:: 
   ...
   You can set Buckit configuration settings in environment variables and using :mc-cmd:`mc admin config set <mc admin config set>`.
...
#. Create a new Single-Node Single-Drive Buckit deployment.
   ...
   *Existing Buckit Client* is :mc:`mc` from the old deployment.
   *New Buckit Client* is :mc:`mc` from the new deployment.

#. Add an alias for the deployment created in the previous step using :mc:`mc alias set` and the new Buckit Client.
...
#. Migrate settings according to the type of deployment:
   ...
   - Filesystem mode deployments provide an S3 access layer for a single Buckit server process and single storage volume.
   ...
   .. tab-set::
	 ...
	 If you use :mc-cmd:`mc admin config set <mc admin config set>` for configuration settings, duplicate the existing settings for the new deployment using the new Buckit Client.
      ...
      .. tab-item:: Filesystem mode
	    ...
	    If it does not, recreate users, policies, lifecycle rules, and buckets manually on the new deployment using the new Buckit Client.
         ...
         a. Export the existing deployment's **configurations**.
            ...
            Use the :mc-cmd:`mc admin config export <mc admin config export>` command with the existing Buckit Client to retrieve the configurations defined for the existing standalone Buckit deployment.
	    ...
	    If :mc-cmd:`~mc admin config import` reports an error for a configuration key, comment it out with ``#`` at the beginning of the relevant line and try again.
            When you are finished migrating the deployment, verify the current syntax for the target Buckit Server version and set any needed keys manually using :mc-cmd:`mc admin config set`.
...
#. Migrate bucket contents with :mc:`mc mirror`.
   ...
   Use :mc:`mc mirror` with the :mc-cmd:`~mc mirror --preserve` and :mc-cmd:`~mc mirror --watch` flags on the standalone deployment to move objects to the new |SNSD| deployment with the existing Buckit Client
