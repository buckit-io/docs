...
A Buckit Tenant consists of a complete set of Kubernetes resources deployed within a namespace that support the Buckit Object Storage service.

This documentation assumes a :ref:`Buckit Operator installation <deploy-minio-operator>` on the target Kubernetes infrastructure.
...
Buckit **strongly recommends** deploying Tenants onto Kubernetes worker nodes with locally attached storage.

The Worker Nodes should meet Buckit's :ref:`hardware checklist <minio-hardware-checklist>` for production environments.

Avoid colocating Buckit tenants onto worker nodes hosting other high-performance softwares, and where necessary to do so ensure you configure the appropriate limits and constraints to guarantee Buckit access to the necessary compute and storage resources.
...
Buckit can typically use any Kubernetes :kube-docs:`Persistent Volume (PV) <concepts/storage/persistent-volumes>` that supports the :kube-docs:`ReadWriteOnce <concepts/storage/persistent-volumes/#access-modes>` access mode.
Buckit's consistency guarantees require the exclusive storage access that ``ReadWriteOnce`` provides.
Additionally, Buckit recommends setting a reclaim policy of ``Retain`` for the PVC :kube-docs:`StorageClass <concepts/storage/storage-classes>`.
...
.. tab-set::
   ...
   .. tab-item:: Amazon EKS
      ...
      Buckit strongly recommends deploying EBS-based PVs with the XFS filesystem.
      Create a StorageClass for the Buckit EBS PVs and set the ``csi.storage.k8s.io/fstype`` `parameter <https://github.com/kubernetes-sigs/aws-ebs-csi-driver/blob/master/docs/parameters.md>`__ to ``xfs``  .

      Buckit recommends the following :github:`EBS volume types <kubernetes-sigs/aws-ebs-csi-driver/blob/master/docs/parameters.md>`:
   ...
   .. tab-item:: Google GKS
      ...
      Buckit Tenants on GKE should use the :gke-docs:`Compute Engine Persistent Disk CSI Driver <how-to/persistent-volumes/gce-pd-csi-driver>` to provision the necessary underlying persistent volumes.

      Buckit recommends the following :gke-docs:`GKE CSI Driver <how-to/persistent-volumes/gce-pd-csi-driver>` storage classes:

      - ``standard-rwo`` (Balanced Persistent SSD)
      - ``premium-rwo`` (Performance Persistent SSD)

      Buckit strongly recommends SSD-backed disk types for best performance.
      For more information on GKE disk types, see :gcp-docs:`Persistent Disks <disks>`.

   .. tab-item:: Azure AKS

      Buckit Tenants on AKS should use the :azure-docs:`Azure Disks CSI driver <azure-disk-csi>` to provision the necessary underlying persistent volumes.

      Buckit recommends the following :aks-docs:`AKS CSI Driver <azure-disk-csi>` storage classes:
...
Starting with v5.0.6, the Buckit Operator supports custom :kube-docs:`init containers <concepts/workloads/pods/init-containers>` for additional pod initialization that may be required for your environment.
