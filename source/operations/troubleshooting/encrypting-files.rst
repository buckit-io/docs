...
You can encrypt the output of the :mc:`bm support inspect` command for enhanced security when transmitting the files to another party.

Encryption
----------

You can choose to encrypt the output zip file for enhanced security with the ``--encrypt`` flag.
Buckit provides a binary to decrypt the file.
...
.. code-block:: shell
   ...
   $ bm support inspect --encrypt play/test123/test*/*/part.*
...
As the output says, Buckit only displays the encryption key this one time, and it cannot be displayed or recovered later.
...
Buckit provides a decryption tool to use on the files generated by :mc:`bm support inspect`.
