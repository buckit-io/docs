...
#. :ref:`Single Node Single Drive <minio-snsd>`, one Buckit server with a single drive or folder for data

   For example, testing on a local PC using a folder on the computer's hard drive. 

#. :ref:`Single Node Multi Drive <minio-snmd>`, one Buckit server with multiple mounted drives or folders for data

   For example, a single container with two or more mounted volumes.

#. :ref:`Multi Node Multi Drive <minio-mnmd>`, multiple Buckit servers with multiple mounted drives or volumes for data
...
If one server pool goes down, Buckit halts I/O to all pools until the cluster resumes normal operations. 
...
Buckit intends decommissioning for use with removing older pools with aged hardware, and not as an operation performed regularly within any deployment.
...
Buckit also implements support for :s3-docs:`multipart uploads <mpuoverview.html>`, where clients can split an object into multiple parts for better throughput and reliability of transmission.
...
If an object becomes corrupted by bit rot, Buckit can automatically :ref:`heal <minio-concepts-healing>` the object depending on the availability of parity shards for the object.

Buckit can also perform bit rot checks and healing using the :ref:`Buckit Scanner <minio-concepts-scanner>`.
...
Buckit divides objects into chunks — called `shards` — and evenly distributes them among each drive in the Erasure Set. 
...
Buckit uses the parity information to confirm the integrity of an object and, if necessary, to restore a lost, missing, or corrupted object shard on a given drive or set of drives.
...
Buckit has one quorum for reading data and a separate quorum for writing data.
