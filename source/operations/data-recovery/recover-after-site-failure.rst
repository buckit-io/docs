...
Buckit can make the loss of an entire site, while significant, a relatively minor incident.
...
:ref:`Site replication <minio-site-replication-overview>` keeps two or more Buckit deployments in sync with IAM policies, buckets, bucket configurations, objects, and object metadata.
...
#. Deploy a new Buckit site following the :ref:`site replication requirements <minio-expand-site-replication>`.
   ...
   - Validate that the new Buckit deployment functions normally and has bidirectional connectivity to the other peer sites.
...
:ref:`Bucket replication <minio-bucket-replication>` can directly restore the site contents by performing a replication from the target bucket(s) to a new Buckit site.
...
#. Deploy a new Buckit site.
#. Set up IAM and users as needed.
#. On the remaining target bucket deployment(s), create bucket replication rule(s) for each bucket to the new Buckit site.
...
Buckit's mirroring copies an object from any S3 compatible storage system.
