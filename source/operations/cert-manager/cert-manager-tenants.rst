...
.. important::
   ...
   - ``tenant-name`` is the name provided to your tenant in the ``metadata.name`` of the Tenant YAML. 
     For this example it is ``mybuckit``.
   ...
   .. code-block:: yaml
      ...
      spec:
        dnsNames:
          ...
          - '*.minio.tenant-1.svc.cluster.local'
          - '*.mybuckit-hl.tenant-1.svc.cluster.local'
          - '*.mybuckit.minio.tenant-1.svc.cluster.local'
        secretName: mybuckit-tls
        issuerRef:
          name: tenant-1-ca-issuer

   .. tip::

      For this example, the Tenant name is ``mybuckit``. 
...
3. Validate the changes took effect:
   ...
   .. code-block:: shell
      ...
      kubectl describe secret/mybuckit-tls -n tenant-1

   .. note::

      - Replace ``tenant-1`` with the namespace for your tenant.
      - Replace ``mybuckit-tls`` with the name of your secret, if different.
