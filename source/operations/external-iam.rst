...
Buckit supports multiple external identity managers through the following IDentity Providers (IDP):
...
- :ref:`Configure Buckit Authentication with KeyCloak <minio-authenticate-using-keycloak>`

Users can authenticate against Buckit using their externally managed credentials and the related :ref:`minio-security-token-service` API.
Once authenticated, Buckit attempts to associate the user with one or more configured :ref:`policies <minio-policy>`.
A user with no associated policies has no permissions on the Buckit deployment.
...
Configuring an external :abbr:`IDP (IDentity Provider)` enables Single-Sign On workflows, where applications authenticate against the external :abbr:`IDP (IDentity Provider)` before accessing Buckit.

Buckit uses :ref:`Policy Based Access Control (PBAC) <minio-access-management>` to define the actions and resources to which an authenticated user has access.
Buckit supports creating and managing :ref:`policies <minio-policy>` which an externally managed user can claim. 

For identities managed by the external OpenID Connect (OIDC) compatible provider, Buckit uses a `JSON Web Token claim <https://datatracker.ietf.org/doc/html/rfc7519#section-4>`__ to identify the :ref:`policy <minio-policy>` to assign to the authenticated user. 

Buckit by default looks for a ``policy`` claim and reads a list of one or more policies to assign. Buckit attempts to match existing policies to those specified in the JWT claim. 
If none of the specified policies exist on the Buckit deployment, Buckit denies authorization for any and all operations issued by that user. 
...
The specified policy claim directs Buckit to attach the policies with names matching ``readwrite_data``, ``read_analytics``, and ``read_logs`` to the authenticated user.
...
Buckit supports using an Active Directory or LDAP (AD/LDAP) service for external
...
the external IDP before accessing Buckit.
...
Buckit queries the configured Active Directory / LDAP server to verify the credentials specified by the application and optionally return a list of groups in which the user has membership.
...
Buckit supports querying the Active Directory / LDAP server for a list of groups in which the authenticated user has membership. 
Buckit attempts to match existing :ref:`policies <minio-policy>` to each group DN and assigns each matching policy to the authenticated user.
