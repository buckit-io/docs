...
Buckit publishes point-in-time metrics using the :prometheus-docs:`Prometheus Data Model <concepts/data_model/>`.
...
The following table lists tutorials for integrating Buckit metrics with select third-party monitoring software.
...
.. list-table::
   ...
   * - :ref:`minio-metrics-collect-using-prometheus`
     - Configure Prometheus to Monitor and Alert for a Buckit deployment

   * - :ref:`minio-metrics-influxdb`
     - Configure InfluxDB to Monitor and Alert for a Buckit deployment.
...
Buckit publishes logs as a JSON document as a ``PUT`` request to each configured endpoint. 
The endpoint server is responsible for processing each JSON document.
Buckit requires explicit configuration of each webhook endpoint and does *not* publish logs to a webhook by default.
...
Buckit exposes unauthenticated endpoints for probing node uptime and  cluster :ref:`high availability <minio-ec-parity>` for simple healthchecks.
