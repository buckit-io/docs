...
`InfluxDB <https://www.influxdata.com/?ref=minio>`__ supports scraping Buckit metrics data for monitoring and alerting.

The procedure on this page documents the following:

- Configuring an InfluxDB service to scrape and display metrics from a Buckit deployment
- Configuring an Alert on a Buckit metric 
...
.. admonition:: Prerequisites
   ...
   - An existing Buckit deployment with network access to the InfluxDB deployment
...
.. important::
   ...
   - You cannot enable authenticated access to the Buckit metrics endpoint via the InfluxDB UI
   - You cannot set a tag for collected metrics (e.g. ``url_tag``) for uniquely identifying the metrics for a given Buckit deployment

   The Telegraf Prometheus plugin also supports Kubernetes-specific features, such as scraping the ``minio`` service for a given Buckit Tenant.

   Configuring Telegraf is out of scope for this procedure. 
   You can use this procedure as general guidance for configuring Telegraf to scrape Buckit metrics.

.. container:: procedure

   1. Configure Public Access to Buckit Metrics

      Set the :envvar:`MINIO_PROMETHEUS_AUTH_TYPE` environment variable to ``"public"`` for all nodes in the Buckit deployment.
      You can then restart the deployment to allow public access to Buckit metrics.
      ...
      Replace ``HOSTNAME`` with the URL of the load balancer or reverse proxy through which you access the Buckit deployment.
      You can alternatively specify any single node as ``HOSTNAME:PORT``, specifying the Buckit server API port in addition to the node hostname.

      The response body should include a list of collected Buckit metrics.

   #. Log into the InfluxDB UI and Create a Bucket

      Select the :influxdb-docs:`Organization <organizations/view-orgs/>` under which you want to store Buckit metrics.

      Create a :influxdb-docs:`New Bucket <organizations/buckets/create-bucket/>` in which to store metrics for the Buckit deployment.

   #. Create a new Scraping Source

      Create a :influxdb-docs:`new InfluxDB Scraper <write-data/no-code/scrape-data/manage-scrapers/create-a-scraper/>`.

      Specify the full URL to the Buckit deployment, including the metrics endpoint:
   ...
   #. Validate the Data
      ...
      Use the :influxdb-docs:`DataExplorer <query-data/execute-queries/data-explorer/>` to visualize the collected Buckit data.

      For example, you can set a filter on ``minio_cluster_capacity_usable_total_bytes`` and ``minio_cluster_capacity_usable_free_bytes`` to compare the total usable against total free space on the Buckit deployment.

   #. Configure a Check

      Create a :influxdb-docs:`new Check <https://docs.influxdata.com/influxdb/v2.4/monitor-alert/checks/create/>` on a Buckit metric.

      The following example check rules provide a baseline of alerts for a Buckit deployment.
