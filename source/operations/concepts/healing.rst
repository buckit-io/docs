...
Healing is Buckit's ability to restore an object that has been damaged, corrupted, or partially lost.
...
- :ref:`parity setting <minio-ec-parity>` for the erasure set
  ...
  :term:`Parity` refers to the number of dedicated recovery shards Buckit creates when writing the object.
  For example, an erasure set may have eight total drives and use three drives during a write for parity.
  In this scenario, Buckit splits an object into 5 data shards and create 3 parity shards.
  Buckit distributes these eight shards across the drives in the erasure set.
  No one drive contains only parity shards or only data shards.
  Instead, Buckit writes shards for each object in a randomized way to distribute reads evenly across drives.

  When Buckit needs to provide the object, it looks for the data shards for the object.
  If any of the data shards are missing or damaged, Buckit uses one or more of the parity shards to restore the object.
  ...
  For this scenario, up to three of data shard parts can be lost or damaged and Buckit can still successfully restore and serve the object. 
...
Buckit automatically checks the consistency of an object's data shards each time you request an object with a ``GET`` or ``HEAD`` operation.
For versioned buckets, Buckit also checks for consistency during ``PUT`` operation.
...
If the object has missing or damaged data shards, Buckit uses the available parity shards to heal the object before serving it as part of the operation.
...
If any parity shards are lost or damaged, Buckit restores the parity shard, provided there are sufficient other parity shards to serve the object.

Healing with the object scanner
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Buckit uses an :ref:`object scanner <minio-concepts-scanner>` to perform a number of tasks related to objects.
...
If any object is found to have lost shards, Buckit heals the object from available shards.
By default, Buckit does *not* check for :term:`bit rot` corruption using the scanner.
