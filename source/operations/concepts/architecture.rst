...
.. meta::
   ...
   :description: Information on Buckit Deployment architecture and topology in production environments

This page provides an overview of Buckit deployment architectures from a production perspective.
...
A production Buckit deployment consists of at least 4 Buckit hosts with homogeneous storage and compute resources.
   ...
   .. figure:: /images/architecture/architecture-4-node-deploy.svg
      :figwidth: 100%
      :alt: 4 Node Buckit deployment with homogeneous storage and compute resources
      :align: center

      Each Buckit host in this pool has matching compute, storage, and network configurations

Buckit provides best performance when using locally-attached storage, such as NVMe or SSD drives attached to a PCI-E controller board on the host machine. 
   ...
   Buckit recommends against caching, either at the drive or the controller layer. 
   ...
   .. figure:: /images/architecture/architecture-one-node-DAS.svg
      :figwidth: 100%
      :alt: Buckit Server diagram of Direct-Attached Storage via SAS to a PCI-E Storage Controller
      :align: center

      Each SSD connects by SAS to a PCI-E-attached storage controller operating in HBA mode

Buckit automatically groups drives in the pool into :ref:`erasure sets <minio-ec-erasure-set>`. 
   Erasure sets are the foundational component of Buckit :ref:`availability and resiliency <minio-availability-resiliency>`. 
   Buckit stripes erasure sets symmetrically across the nodes in the pool to maintain even distribution of erasure set drives.
   Buckit then partitions objects into data and parity shards based on the deployment :ref:`parity <minio-ec-parity>` and distributes them across an erasure set.

   For a more complete discussion of Buckit redundancy and healing, see :ref:`minio-erasure-coding` and :ref:`minio-concepts-healing`.
   ...
   .. figure:: /images/architecture/architecture-erasure-set-shard.svg
      ...
      With the maximum parity of ``EC:8``, Buckit shards the object into 8 data and 8 parity blocks, distributing them across the drives in the erasure set.
      All erasure sets in this pool have the same stripe size and shard distribution.

Buckit uses a deterministic hashing algorithm based on object name and path to select the erasure set for a given object.
   For each unique object namespace ``BUCKET/PREFIX/[PREFIX/...]/OBJECT.EXTENSION``, Buckit always selects the same erasure set for read/write operations.
   Buckit handles all routing within pools and erasure sets, making the select/read/write process entirely transparent to applications.
   ...
   .. figure:: /images/architecture/architecture-erasure-set-retrieve-object.svg
      ...
      Buckit reconstructs objects from data or parity shards transparently before returning the object to the requesting client.

Each Buckit server has a complete picture of the distributed topology, such that an application can connect and direct operations against any node in the deployment.
   The Buckit responding node automatically handles routing internal requests to other nodes in the deployment *and* returning the final response to the client.
   ...
   Production environments should instead deploy a load balancer or similar network control plane component to manage connections to the Buckit deployment.
   ...
   .. figure:: /images/architecture/architecture-load-balancer-8-node.svg
      :figwidth: 100%
      :alt: Diagram of an eight node Buckit deployment behind a load balancer
      :align: center
...
You can expand a Buckit deployment's available storage through :ref:`pool expansion <expand-minio-distributed>`.
   Each pool consists of an independent group of nodes with their own erasure sets.
   Buckit must query each pool to determine the correct erasure set to which it directs read and write operations, such that each additional pool adds increased internode traffic per call.
   ...
   If you modify the Buckit topology through pool expansion, you can update your applications by modifying the load balancer to include the new pool's nodes.
   Applications can continue using the load balancer address for the Buckit deployment without any updates or modifications.
   This ensures even distribution of requests across all pools, while applications continue using the single load balancer URL for Buckit operations.
...
Client applications can use any S3-compatible SDK or library to interact with the Buckit deployment.
   ...
   Buckit recommends using S3-compatible SDKs or libraries which perform the signature calculation automatically as part of operations.
...
Buckit :ref:`site replication <minio-site-replication-overview>` provides support for synchronizing distinct independent deployments.
   ...
   .. figure:: /images/architecture/architecture-multi-site.svg
      :figwidth: 100%
      :alt: Diagram of a multi-site deployment with three Buckit peer site

      A Buckit multi-site deployment with three peers.
...
Deploying a global load balancer or similar network appliance with support for site-to-site failover protocols is critical to the functionality of multi-site deployments.
   ...
   Buckit replication handles transient failures by queuing objects for replication.
