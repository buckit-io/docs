...
The :mc:`bm admin policy` command supports creation and management of policies on the Buckit deployment. 
...
Use :mc:`bm admin policy attach` to associate a policy to a 
...
.. policy-action:: s3:BypassGovernanceRetention
   ...
   Controls access to the following S3 API operations on objects locked under :mc-cmd:`GOVERNANCE <bm retention set MODE>` retention mode:
...
for :mc:`bm admin` operations. These actions are *only* valid for
...
:mc:`bm admin` :ref:`actions <minio-policy-mc-admin-actions>`.
