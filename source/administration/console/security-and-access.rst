...
- Create and manage :ref:`user credentials <minio-console-admin-identity>` or groups with the built-in Buckit IDP, connect to one or more OIDC provider, or add an AD/LDAP provider for SSO.
...
The :guilabel:`Policies` section displays all :ref:`policies <minio-policy>` on the Buckit deployment. 
...
Refer to :ref:`Policy Based Action Control <minio-policy>` for details on managing access in Buckit with policies.
...
- Select :guilabel:`+ Create Policy` to create a new Buckit Policy.
...
The :guilabel:`Identity` section provides a management interface for :ref:`Buckit-Managed users <minio-users>`.
...
The :guilabel:`Users` section displays all Buckit-managed  :ref:`users <minio-users>` on the deployment.
...
- Select :guilabel:`Create User` to create a new Buckit-managed user. 
...
The :guilabel:`Groups` section displays all :ref:`groups <minio-groups>` on the Buckit deployment. 

This section is not visible for deployments using an external identity manager such as Active Directory or an OIDC-compatible provider.

- Select :guilabel:`Create Group` to create a new Buckit Group. 
...
Buckit supports using an :ref:`OpenID Connect (OIDC) compatible IDentity Provider (IDP) <minio-external-identity-management-openid>` for external management of user identities.
...
Configuring an external IDP enables Single-Sign On workflows, where applications authenticate against the external IDP before accessing Buckit.

Use the the screens in this section to view, add, or edit OIDC configurations for the deployment.
Buckit supports any number of active OIDC configurations.
...
Buckit supports using an :ref:`Active Directory or LDAP (AD/LDAP) <minio-external-identity-management-ad-ldap>` service for external management of user identities. 
Configuring an external IDentity Provider (IDP) enables Single-Sign On (SSO) workflows, where applications authenticate against the external IDP before accessing Buckit.

Use the the screens in this section to view, add, or edit an LDAP configuration for the deployment.
Buckit only supports one active LDAP configuration.

Buckit queries the Active Directory / LDAP server to verify the client-specified credentials. 
Buckit also performs a group lookup on the AD/LDAP server if configured to do so.
