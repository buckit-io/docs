...
Buckit supports server-side and client-side replication of objects between source
and destination buckets.

:ref:`Server-Side Bucket Replication <minio-bucket-replication-serverside>`
  Configure per-bucket rules for automatically synchronizing objects between Buckit deployments.
  ...
  Buckit applies rules as part of object write operations (e.g. ``PUT``) and automatically synchronizes new objects *and* object mutations, such as new object versions or changes to object metadata.
  
  Buckit server-side bucket replication only supports a Buckit cluster on an identical release for the remote replication target.
...
.. admonition:: Bucket vs Site Replication
   ...
   - Bucket Replication synchronizes data at the bucket level, such as bucket prefix paths and objects.
     ...
     You can configure bucket replication at any time, and the remote Buckit deployments may have pre-existing data on the replication target buckets.
   ...
   - Site Replication extends bucket replication to include :ref:`IAM <minio-authentication-and-identity-management>`, security tokens, access keys, and bucket-level configurations.
     ...
     Site replication is typically configured when initially deploying the Buckit peer sites.
...
Buckit server-side bucket replication is an automatic bucket-level configuration that synchronizes objects between a source and destination bucket. 
Buckit server-side replication *requires* the source and destination bucket be two separate Buckit clusters running the same Buckit Server version.

For each write operation to the bucket, Buckit checks all configured replication rules for the bucket and applies the matching rule with highest configured priority. 
Buckit synchronizes new objects *and* object mutations, such as new object versions or changes to object metadata. 
This includes metadata operations such as enabling or modifying object locking or retention settings.

Buckit server-side bucket replication is functionally similar to Amazon S3 replication while adding the following Buckit-only features:
...
- Multi-Site replication of objects between three or more Buckit deployments
...
Buckit skips synchronizing those objects whose remote copy exactly match the
...
permanently disconnected from the remote Buckit deployment. For tiering
...
Buckit supports replicating :ref:`delete <minio-object-delete>` operations, where Buckit synchronizes
...
For delete marker replication, Buckit begins the replication process after
a delete operation creates the delete marker. Buckit uses the 
...
replication configurations, Buckit may produce duplicate delete markers if
...
For replicating the deletion of a specific object version, Buckit marks the
object version as ``PENDING`` until replication completes. Once the remote
target deletes that object version, Buckit deletes the object on the source.
...
delete operation. Buckit uses the ``X-Minio-Replication-Delete-Status`` for
tracking delete version replication status.

Buckit only replicates explicit client-driven delete operations. Buckit does *not*
...
.. admonition:: Buckit Trims Empty Object Prefixes on Source and Remote Bucket
   :class: note, dropdown

   If a delete operation removes the last object in a bucket prefix, Buckit
   ...
   - A command removes ``myphoto.jpg``. Buckit automatically trims the empty
     ``/janaury`` prefix. 
   
   - A command then removes the ``myotherphoto.jpg``. Buckit automatically
     trims the ``/february`` prefix *and* the now-empty ``/2021`` prefix. 
   
   - A command removes the ``NewYears.jpg`` object. Buckit leaves the 
...
Buckit by default replicates existing objects in the source bucket to the configured remote, similar to `AWS: Replicating existing objects between S3 buckets <https://aws.amazon.com/blogs/storage/replicating-existing-objects-between-s3-buckets/>`__ without the overhead of contacting technical support.

Buckit marks all objects or object prefixes that satisfy the replication rules as eligible for synchronization to the remote cluster and bucket.
Buckit only excludes those objects without a version ID, such as those objects written before enabling versioning on the bucket.
...
Buckit uses a replication queuing system with multiple concurrent replication
workers operating on that queue. Buckit continuously works to replicate and
...
.. list-table::
   ...
   * - ``PENDING``
     - The object has not yet been replicated. Buckit applies this state
       if the object meets one of the configured replication rules on the
       bucket. Buckit continuously scans for ``PENDING`` objects not yet in the
   ...
   * - ``FAILED``
     - The object failed to replicate to the remote cluster. 
       ...
       Buckit continuously scans for ``FAILED`` objects not yet in the
       replication queue and adds them to the queue as space is available.
