...
Buckit uses a :ref:`policy based access control <minio-policy>` system for access management.
...
After verifying the user or service account has permission to perform the ``DELETE`` operation, Buckit permanently removes the object.
...
In this scenario, Buckit does not actually remove the object or any of its versions from the disk.
...
.. warning::
   ...
   Buckit removes the specified version from the drive and **cannot** retrieve it.
...
When more versions exist than the rule specifies, or when a version is older than specified, Buckit permanently removes the object version from the drive.
...
Because of this, object versions that meet the requirements for expiration may not immediately be removed from Buckit.
...
Buckit protects objects subject to a :ref:`locking rule <minio-object-locking>` from being overwritten or deleted.
...
Buckit protects object versions subject to a lock from being overwritten or deleted until the lock expires or is removed.

Replicated Objects
------------------

Replication duplicates objects from one location to another.
Buckit supports replication at the bucket level or the cluster ("site") level.
...
For clusters with :ref:`multi-site replication <minio-site-replication-overview>` enabled, Buckit replicates all ``delete`` operations performed on any cluster to each of the other clusters in the peer group.

Delete behavior on any single peer follows the same processes as any Buckit deployment.

Bucket Replication
~~~~~~~~~~~~~~~~~~

With :ref:`bucket replication <minio-bucket-replication>`, Buckit supports replicating delete operations between a source bucket and a configured remote bucket.
Buckit synchronizes deleting specific object versions *and* new  :s3-docs:`delete markers <delete-marker-replication.html>`. 
...
Buckit uses the ``X-Minio-Replication-DeleteMarker-Status`` metadata field for tracking  delete marker replication status. 
In :ref:`active-active <minio-bucket-replication-serverside-twoway>` replication configurations, Buckit may produce duplicate delete markers if both clusters concurrently create a delete marker for an object *or* if one or both clusters were down before the replication event synchronized.

For replicating the deletion of a specific object version, Buckit marks the object version as ``PENDING`` until replication completes. 
Once the remote target deletes that object version, Buckit deletes the object version on the source.
...
Buckit uses the ``X-Minio-Replication-Delete-Status`` for tracking delete version replication status.

Buckit only replicates explicit client-driven delete operations. 
Buckit does *not* replicate objects deleted by :ref:`lifecycle management expiration rules <minio-lifecycle-management-expiration>`. 
...
.. admonition:: Buckit Trims Empty Object Prefixes on Source and Remote Bucket
   ...
   - A command removes ``myphoto.jpg``. 
     Buckit automatically trims the empty ``/january/`` prefix. 
   
   - A command then removes the ``myotherphoto.jpg``. 
     Buckit automatically trims the ``/february/`` prefix *and* the now-empty ``/2021`` prefix. 
   
   - A command removes the ``NewYears.jpg`` object. 
     Buckit leaves the ``/NYE21/`` prefix remains in place since it was *explicitly* created.
