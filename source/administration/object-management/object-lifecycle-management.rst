...
Buckit derives it's behavior and syntax from :s3-docs:`S3 lifecycle <object-lifecycle-mgmt.html>` for compatibility in migrating workloads and lifecycle rules from S3 to Buckit.
For example, you can export S3 lifecycle management rules and import them into Buckit or vice-versa. 
Buckit uses JSON to describe lifecycle management rules and may require conversion to or from XML as part of importing S3 lifecycle rules. 
...
Buckit supports creating object transition lifecycle management rules, where Buckit can automatically move an object to a remote storage "tier". 
Buckit supports any of the following remote tier targets:

- :ref:`Buckit <minio-lifecycle-management-transition-to-minio>`
...
Buckit object transition supports use cases like moving aged data from Buckit clusters in private or public cloud infrastructure to low-cost private or public cloud storage solutions.
...
.. important::
   ...
   Buckit Object Transition supports cost-saving strategies around moving older or aged data to cost-optimized remote storage tiers, such as cloud storage or high-density HDD storage.

   Buckit Object Transition does **not** provide backup and recovery functionality.
   You cannot use the remote tier as a recovery source in the event of data loss in Buckit.
...
Buckit lifecycle management supports expiring objects on a bucket. 
...
Buckit adopts :s3-docs:`S3 behavior <intro-lifecycle-rules.html#intro-lifecycle-rules-actions>` for expiration rules on :ref:`versioned buckets <minio-bucket-versioning>`. 
Buckit has several default behaviors for versioned buckets:
...
Buckit uses a built-in :ref:`scanner <minio-concepts-scanner>` to actively check objects against all configured lifecycle management rules. 
