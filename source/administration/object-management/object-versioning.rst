...
Buckit supports keeping multiple "versions" of an object in a single bucket.

When enabled, versioning allows Buckit to keep multiple iterations of the same object.
...
Buckit versioning protects from unintended overwrites and deletions while providing support for "undoing" a write operation. 
...
Buckit marks the "latest" version of the object that clients retrieve by default. 
...
Review each of the four images in this series to see how Buckit retrieves objects in a versioned bucket.
...
.. card-carousel:: 1
   ...
   .. card:: Object with Single Version
      ...
      Buckit adds a unique version ID to each object as part of write operations.
   ...
   .. card:: Object with Multiple Versions
      ...
      Buckit retains all versions of an object and marks the most recent
...
Buckit uses the full namespace (the bucket and path to an object) for each object
...
Buckit only enforces versioning with a specific namespace and therefore
considers each ``object.blob`` above as distinct and unique.

Versioning and Storage Capacity
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Buckit does not perform incremental or differential-type versioning. For
...
appends 100MB of data to the log and uploads to Buckit. Buckit would then contain
...
Buckit supports configuring configuring :ref:`object lifecycle management rules 
...
:ref:`Buckit Object Expiration <minio-lifecycle-management-expiration>` for 
more information.
...
Buckit generates a unique and immutable identifier for each versioned object as
...
Buckit can utilize :ref:`Lifecycle Management expiration rules <minio-lifecycle-management-expiration>` to automatically remove versioned objects permanently.
Otherwise, use manual ``DELETE`` operations to permanently remove non-current versioned objects or ``DeleteMarker`` objects.

.. admonition:: Buckit Implements Idempotent Delete Markers
   :class: note
   ...
   Buckit diverges from standard S3 implementation by avoiding this potential duplication of delete markers.
   When processing a ``Delete`` request with no version identifier, Buckit creates at most one Delete Marker for the specified object.
   Buckit **does not** share S3's behavior in creating multiple sequential delete markers.
...
.. card-carousel:: 1
   ...
   .. card:: Reading a Deleted Object
      ...
      Clients by default retrieve the "latest" object version. Buckit returns
...
You can exclude certain :ref:`prefixes <minio-admin-concepts-organize-objects>` from versioning using the :ref:`Buckit Client <minio-client>`.
...
.. admonition:: Replication and Object Locking Require Versioning
   ...
   Buckit requires versioning to support :term:`replication`.
...
You can exclude folders from versioning using the :ref:`Buckit Client <minio-client>`.
...
.. admonition:: Replication and Object Locking Require Versioning
   ...
   Buckit does not support excluding folders from versioning on buckets with :ref:`object locking enabled <minio-object-locking>`.
...
Buckit does not remove or otherwise alter existing versioned objects as part of suspending versioning. 
