...
Buckit Server-Side Encryption (SSE) protects objects as part of write operations, allowing clients to take advantage of server processing power to secure objects at the storage layer (encryption-at-rest). 
SSE also provides key functionality to regulatory and compliance requirements around secure locking and erasure.

Buckit SSE uses the :kes-docs:`Buckit Key Encryption Service (KES) <>` and an external Key Management Service (KMS) for performing secured cryptographic operations at scale. 
Buckit also supports client-managed key management, where the application takes full responsibility for creating and managing encryption keys for use with Buckit SSE. 

Buckit SSE is feature and API compatible with :s3-docs:`AWS Server-Side Encryption <server-side-encryption.html>` and supports the following encryption strategies:
...
.. tab-set::
   ...
   .. tab-item:: SSE-KMS *Recommended*
      ...
      Buckit supports enabling automatic SSE-KMS encryption of all objects written to a bucket using a specific External Key (EK) stored on the external |KMS|. 
      ...
      Buckit encrypts backend data as part of enabling server-side encryption.
      ...
      For a tutorial on enabling SSE-KMS in a local (non-production) Buckit Deployment, see :ref:`minio-encryption-sse-kms-quickstart`. 
      For production Buckit deployments, use one of the following guides:
   ...
   .. tab-item:: SSE-S3
      ...
      Buckit supports enabling automatic SSE-S3 encryption of all objects
      written to a bucket using an |EK| stored on the external |KMS|. Buckit
      ...
      For a tutorial on enabling SSE-s3 in a local (non-production) Buckit
      Deployment, see :ref:`minio-encryption-sse-s3-quickstart`. For
      production Buckit deployments, use one of the following guides:
...
Buckit requires access to the Encryption Key (EK) *and* external Key Management
...
- Seal the |KMS| such that it cannot be accessed by Buckit server anymore. This
