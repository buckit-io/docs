...
The Buckit Security Token Service (STS) ``AssumeRoleWithCustomToken`` API endpoint generates a token for use with the :ref:`minio-external-identity-management-plugin`.
...
.. list-table::
   ...
   * - ``Token``
     ...
     - *Required*
       ...
       Buckit expects the identity manager to parse the token and determine whether to authenticate client requests using that token.
   ...
   * - ``RoleArn``
     ...
     - *Required*
       ...
       Note that Buckit automatically prepends ``idmp-`` to a configured ``ROLE_ID`` when generating the RoleArn.
