...
The Buckit Security Token Service (STS) ``AssumeRoleWithWebIdentity`` API 
...
<minio-external-identity-management-openid-configure>`. This page documents the Buckit 
...
The Buckit STS ``AssumeRoleWithWebIdentity`` API endpoint is modeled
after the
...
documents the Buckit-specific syntax and links out to the AWS reference for
all shared elements.
...
.. list-table::
   ...
   * - ``DurationSeconds``
     ...
     - *Optional*
       ...
       If ``DurationSeconds`` is omitted, Buckit checks the JWT token for an
   ...
   * - ``Policy``
     ...
     - *Optional*
       ...
       See :ref:`minio-access-management` for more information on Buckit
   ...
   * - ``RoleArn``
     ...
     - *Optional*   
       ...
       If you do not supply a RoleArn, Buckit attempts to authorize through a JWT-based claim.
...
Specifically, Buckit returns an ``AssumeRoleWithWebIdentityResult`` object,
where the ``AssumedRoleUser.Credentials`` object contains the temporary
credentials generated by Buckit:
