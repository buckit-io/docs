...
The Buckit Security Token Service (STS) ``AssumeRoleWithLDAPIdentity`` API 
endpoint generates temporary access credentials using Active Directory
or LDAP user credentials. This page documents the Buckit 
...
The Buckit STS ``AssumeRoleWithLDAPIdentity`` API endpoint is modeled
after the
...
Specifically, Buckit returns an ``AssumeRoleWithLDAPIdentityResult`` object,
