...
The :mc-cmd:`bm admin trace` command displays API operations occurring on the target Buckit deployment.
...
Use :mc-cmd:`bm admin trace` to monitor API operations on a Buckit deployment:
...
- Replace :mc-cmd:`ALIAS <bm admin trace TARGET>` with the :mc-cmd:`alias <bm alias>` of the Buckit deployment.

See Calls that Return 503 Errors
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc-cmd:`bm admin trace` to monitor API operations that return a service unavailable 503 error:
...
Use :mc-cmd:`bm admin trace` to monitor activity for a specific path:
...
Use :mc-cmd:`bm admin trace` to monitor responses over a specific size:
...
Use :mc-cmd:`bm admin trace` to monitor long operations:
...
:mc-cmd:`bm admin trace` has the following syntax:
...
:mc-cmd:`bm admin trace` supports the following argument:

.. mc-cmd:: TARGET

   Specify the :mc:`alias <bm alias>` of a configured Buckit deployment for which to monitor API operations.
...
.. mc-cmd:: --call
   ...
   .. code-block:: shell
      ...
      bm admin trace --call scanner TARGET
   ...
   If not specified, Buckit returns call types of ``s3``.
...
.. mc-cmd:: --filter-request
   ...
   Trace client operations or calls with request size greater than the specified :mc-cmd:`~bm admin trace --filter-size` value.

   Must be used with :mc-cmd:`~bm admin trace --filter-size` flag.

.. mc-cmd:: --filter-response

   Trace client operations or calls with response size greater than the specified :mc-cmd:`~bm admin trace --filter-size` value.
...
.. mc-cmd:: --filter-size
   ...
   Must be used with either :mc-cmd:`~bm admin trace --filter-request` or :mc-cmd:`~bm admin trace --filter-response` flag.
...
.. mc-cmd:: --request-query
   ...
   This debug option should only be used at the direction of Buckit Support.
...
.. mc-cmd:: --response-threshold
   ...
   If not specified, Buckit returns calls with a response time greater than 5ms.
