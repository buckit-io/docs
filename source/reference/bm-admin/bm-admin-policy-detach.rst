...
Remove one or more IAM policies from either a :ref:`Buckit-managed user or a group <minio-users>`. 

.. end-mc-admin-policy-detach-desc

Exactly one :mc-cmd:`~bm admin policy detach --user` or one :mc-cmd:`~bm admin policy detach --group` is required.
...
.. important::
   ...
   For detaching policies from Active Directory/LDAP users or groups, use :mc-cmd:`bm idp ldap policy detach`.

Parameters
~~~~~~~~~~

The :mc-cmd:`bm admin policy detach` command accepts the following arguments:

.. mc-cmd:: TARGET
   :required:

   The :mc-cmd:`alias <bm alias>` of a configured Buckit deployment with the user or group for which you want to detach one or more policies.
