...
The :mc:`bm admin prometheus metrics` command prints Prometheus metrics for a cluster.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] admin prometheus metrics  \
                                           ALIAS                                           \
                                           [TYPE]                                          \
                                           [--api_version v3]                              \
                                           [--bucket <bucket name>]
...
.. mc-cmd:: ALIAS
   ...
   The :mc:`alias <bm alias>` of a configured Buckit deployment for which the command prints metrics.
...
.. mc-cmd:: --bucket
   ...
   Requires :mc-cmd:`~bm admin prometheus metrics --api-version`.
   ...
   .. code-block:: shell
      ...
      bm admin prometheus metrics ALIAS api --bucket mybucket --api-version v3
...
Use :mc-cmd:`bm admin prometheus metrics --api-version v3 <bm admin prometheus metrics --api-version>` to print all available v3 metrics and their current values for a Buckit deployment:

.. code-block:: shell
   :class: copyable

   bm admin prometheus metrics ALIAS --api-version v3

- Replace ``ALIAS`` with the :mc-cmd:`alias <bm alias>` of the Buckit deployment.

To print a specific type of metrics, include the :mc-cmd:`~bm admin prometheus metrics TYPE`.
The following prints all scanner metrics for a deployment:

.. code-block:: shell
   :class: copyable

   bm admin prometheus metrics ALIAS scanner --api-version v3

Print v3 API or bucket replication metrics
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Certain v3 metric types accept a :mc-cmd:`~bm admin prometheus metrics --bucket` parameter to specify the bucket for which to print metrics.
The following example prints v3 replication metrics for bucket ``mybucket``:

.. code-block:: shell
   :class: copyable

   bm admin prometheus metrics ALIAS replication --bucket mybucket --api-version v3
...
By default, :mc-cmd:`bm admin prometheus metrics` prints v2 cluster metrics:

.. code-block:: shell
   :class: copyable

   bm admin prometheus metrics ALIAS
...
To print another type of v2 metrics, specify the desired :mc-cmd:`~bm admin prometheus metrics TYPE`.
The following example prints v2 bucket metrics:

.. code-block:: shell
   :class: copyable

   bm admin prometheus metrics ALIAS bucket
