...
The :mc-cmd:`bm admin info` command displays information on a Buckit server.
For distributed Buckit deployments, :mc-cmd:`bm admin info` displays information
for each Buckit server in the deployment.
...
:mc-cmd:`bm admin info` has the following syntax:
...
Specify the :mc-cmd:`alias <bm alias>` of a configured Buckit deployment as the ``TARGET``.
