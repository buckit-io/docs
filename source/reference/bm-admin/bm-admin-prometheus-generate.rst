...
The :mc:`bm admin prometheus generate` command generates a metrics scraping configuration file for use with `Prometheus <https://prometheus.io/>`__.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] admin prometheus generate                                        \
                                           ALIAS                                           \
                                           [TYPE]                                          \
                                           [--api_version v3]                              \
                                           [--bucket <bucket name>]
...
.. mc-cmd:: ALIAS
   ...
   The :mc:`alias <bm alias>` of a configured Buckit deployment for which the command generates a Prometheus-compatible configuration file.
...
.. mc-cmd:: --bucket
   ...
   Requires :mc-cmd:`~bm admin prometheus generate --api-version`.
   ...
   .. code-block:: shell
      ...
      bm admin prometheus generate ALIAS api --bucket mybucket --api-version v3
...
Use :mc-cmd:`bm admin prometheus generate --api-version v3 <bm admin prometheus generate --api-version>` to generate a scrape configuration that collects all v3 metrics for a Buckit deployment:

.. code-block:: shell
   :class: copyable

   bm admin prometheus generate ALIAS --api-version v3
...
.. code-block:: shell
   ...
   bm admin prometheus generate ALIAS cluster --api-version v3
...
To generate a configuration for a :mc-cmd:`different metric type <bm admin prometheus generate TYPE>`, replace ``cluster`` with the desired type.
...
.. code-block:: shell
   ...
   bm admin prometheus generate ALIAS replication --bucket mybucket --api-version v3
...
By default, :mc-cmd:`bm admin prometheus generate` generates a scrape configuration for v2 cluster metrics:
