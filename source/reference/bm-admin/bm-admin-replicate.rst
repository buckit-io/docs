...
The :mc-cmd:`bm admin replicate` command creates and manages :ref:`site replication <minio-site-replication-overview>` for a set of Buckit peer sites.

Site replication mimics an active-active bucket replication, but for multiple Buckit deployments.
...
Where :ref:`bucket replication <minio-bucket-replication>` manages the mirroring of particular buckets or objects from one location to another within a deployment or across deployments, site replication continuously mirrors an entire Buckit site to other sites.

:mc-cmd:`bm admin replicate` only supports site replication for :ref:`distributed deployments <deploy-minio-distributed>` when configuring site replication.
...
Site replication fully synchronizes versioned objects, compared to :mc:`bm mirror` which operates only on the latest version of an object
...
The :mc-cmd:`bm admin replicate` command has the following subcommands:
...
.. mc-cmd:: add
   ...
   .. tab-set::
      ...
      .. tab-item:: EXAMPLES
         ...
         Consider a multi-site Buckit topology with three separate Buckit deployments using the following :ref:`aliases <alias>`: ``minio1``, ``minio2``, and ``minio3``. 
         ...
         If any existing sites are unreachable, first remove the unreachable sites with :mc-cmd:`bm admin replicate rm`, then proceed with the site replication expansion.
   ...
   .. mc-cmd:: ALIAS
      ...
      The :ref:`alias <alias>` of a Buckit deployment to include in site replication.

      At least two Buckit deployment aliases are required to create a site replication. 
...
.. mc-cmd:: update
   ...
   .. versionchanged:: RELEASE.2023-01-11T03-14-16Z
      ...
      ``bm admin replicate edit`` renamed to ``bm admin replicate update``.
...
.. mc-cmd:: rm, remove
   ...
   .. versionchanged:: RELEASE.2023-01-11T03-14-16Z
      ...
      The ``bm admin replicate remove`` subcommand renamed to ``bm admin replicate rm``.
   ...
   .. mc-cmd:: TARGET
      ...
      The :ref:`alias <alias>` of an active Buckit deployment participating in the site replication to target.
      Do not use an alias of a deployment to be removed, unless removing all sites from site replication.

   .. mc-cmd:: ALIAS
      :optional:

      The :ref:`alias <alias>` of an active Buckit deployment to remove from a site replication configuration.
...
.. mc-cmd:: info
   ...
   .. mc-cmd:: ALIAS
      ...
      The :ref:`alias <alias>` of an active Buckit deployment in the site replication configuration.
...
.. mc-cmd:: resync
   ...
   .. mc-cmd:: alias1
      ...
      The :ref:`alias <alias>` of an active Buckit deployment in the site replication configuration with the data you want to resync to another site.

   .. mc-cmd:: alias2

      The :ref:`alias <alias>` of an active Buckit deployment in the site replication configuration that needs data resynced from another site.
