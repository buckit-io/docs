...
.. important::
   ...
   For viewing policies for Active Directory/LDAP users or groups, use :mc-cmd:`bm idp ldap policy entities`.

Parameters
~~~~~~~~~~

The :mc-cmd:`bm admin policy entities` command accepts the following arguments:

.. mc-cmd:: TARGET
   :required:

   The :mc-cmd:`alias <bm alias>` of a configured Buckit deployment on which to add the new policy.
...
.. code-block:: shell
   ...
   bm admin policy entities play/ --group auditors --group accounting
