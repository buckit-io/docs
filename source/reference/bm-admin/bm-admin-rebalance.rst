...
The :mc-cmd:`bm admin rebalance` command allows starts, monitors, or stops a rebalancing operation on a Buckit deployment.
Rebalancing redistributes objects across all pools in the deployment.

.. end-mc-admin-rebalance-desc

Buckit does not automatically rebalance objects when adding a new server pool.
Instead, Buckit :ref:`writes new objects <minio-writing-files>` to the pool with relatively more free space compared to the other available pools on the deployment.
Triggering a manual rebalancing procedure prompts Buckit to scan the entire deployment and move objects as necessary to achieve a similar available free space across all pools.
...
.. code-block:: shell
   ...
   bm admin trace --call rebalance ALIAS
...
The :mc-cmd:`bm admin rebalance` command has the following subcommands:
...
.. list-table::
   ...
   * - :mc-cmd:`bm admin rebalance start`
     - Starts a rebalance operation on a Buckit deployment.

   * - :mc-cmd:`bm admin rebalance status`
     - Outputs the current status of an in-progress rebalance operation.

   * - :mc-cmd:`bm admin rebalance stop`
...
.. mc-cmd:: start
   ...
   Start a rebalance operation for a Buckit deployment.

   .. tab-set::

      .. tab-item:: EXAMPLES

         Consider a Buckit deployment with two pools with an assigned alias of ``minio1``.
         One pool has 250 GB of free space while the other pool has 3 TB of free space.

         The :mc:`bm admin rebalance` command shifts objects from the pool with less free space to the pool with more free space so that there is roughly equal free space on both pools.

         .. code-block:: shell
            :class: copyable

            bm admin rebalance start minio1
      ...
      .. tab-item:: SYNTAX
         ...
         .. code-block:: shell
            ...
            bm [GLOBALFLAGS] admin rebalance start ALIAS

         - Replace ALIAS with the :ref:`alias <alias>` of a Buckit deployment to rebalance.
   ...
   .. tab-set::
      ...
      .. tab-item:: EXAMPLE
         ...
         .. code-block:: shell
            ...
            bm admin rebalance status minio1
      ...
      .. tab-item:: SYNTAX
         ...
         .. code-block:: shell
            ...
            bm [GLOBALFLAGS] admin rebalance ALIAS
   ...
   .. tab-set::
      ...
      .. tab-item:: EXAMPLES
         ...
         .. code-block:: shell
            ...
            bm admin rebalance stop minio1
      ...
      .. tab-item:: SYNTAX
         ...
         .. code-block:: shell
            ...
            bm [GLOBALFLAGS] admin rebalance stop ALIAS
