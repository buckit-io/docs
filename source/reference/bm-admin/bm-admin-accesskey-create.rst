...
The :mc-cmd:`bm admin accesskey create` command adds a new access key and secret key pair for an existing Buckit user.
...
.. admonition:: Access keys for OpenID Connect or AD/LDAP users
   ...
   This command is for access keys for users created directly on the Buckit deployment and not managed by a third party solution.

   To generate access keys for :ref:`Active Directory/LDAP users <minio-external-identity-management-ad-ldap>`, use :mc:`bm idp ldap accesskey create`.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] admin accesskey create                    \
                                          ALIAS                     \
                                          [USER]                    \
                                          [--access-key string]     \
                                          [--secret-key string]     \
...
.. mc-cmd:: USER
   ...
   If not specified, Buckit generates an access key/secret key pair for the authenticated user.
...
.. mc-cmd:: --expiry
   ...
   Mutually exclusive with :mc-cmd:`~bm admin accesskey create --expiry-duration`.
...
.. mc-cmd:: --expiry-duration
   ...
   Mutually exclusive with :mc-cmd:`~bm admin accesskey create --expiry`.
