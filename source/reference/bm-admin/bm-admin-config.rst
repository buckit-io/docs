...
The :mc:`bm admin config` command manages configuration settings for the
:mc:`minio` server.
...
.. mc-cmd:: set
   ...
   Sets a :ref:`configuration key <minio-server-configuration-settings>` on the Buckit deployment.
   Configurations defined by environment variables override configurations defined by this command.

.. mc-cmd:: get
   :fullpath:

   Gets a :ref:`configuration key <minio-server-configuration-settings>` on the Buckit deployment created using `bm admin config set`.

.. mc-cmd:: export
   :fullpath:

   Exports any configuration settings created using `bm admin config set`.

.. mc-cmd:: history
   :fullpath:

   Lists the history of changes made to configuration keys by `bm admin config`.
...
.. mc-cmd:: import
   ...
   Imports configuration settings exported using `bm admin config export`.
