...
.. important::
   ...
   As of Buckit Manager CLI RELEASE.2024-10-08T09-37-26Z, use the :mc:`bm admin accesskey ls` command to list access keys for built-in Buckit IDP users.

   For access keys for AD/LDAP users, use the :mc:`bm idp ldap accesskey ls` command.
...
The :mc:`bm admin user svcacct ls` command lists all access keys associated to the specified user.

.. end-mc-admin-svcacct-list-desc

The alias :mc:`bm admin user svcacct list` has equivalent functionality to :mc:`bm admin user svcacct ls`.
