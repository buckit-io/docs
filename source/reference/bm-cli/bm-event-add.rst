...
The :mc:`bm event add` command adds event notification triggers to a bucket.

.. end-mc-event-add-desc

Buckit automatically sends triggered events to the configured
...
.. mc-cmd:: ALIAS
   ...
   The Buckit :ref:`alias <alias>` and bucket to which the command adds the new event notification. 
   For example:
...
.. mc-cmd:: ARN
   ...
   The Buckit server outputs an ARN for each configured notification target at server startup. 
...
.. mc-cmd:: --prefix
   ...
   The bucket prefix in which the specified :mc-cmd:`~bm event add --event` can trigger a bucket notification.

   For example, given a :mc-cmd:`~bm event add ALIAS` of ``play/mybucket`` and a :mc-cmd:`~bm event add --prefix` of ``photos``, only events in ``play/mybucket/photos`` trigger bucket notifications.
...
.. mc-cmd:: --suffix
   ...
   The bucket suffix in which the specified :mc-cmd:`~bm event add --event` can trigger a bucket notification.

   For example, given a :mc-cmd:`~bm event add ALIAS` of ``play/mybucket`` and a :mc-cmd:`~bm event add --suffix` of ``.jpg``, only events in ``play/mybucket/*.jpg`` trigger bucket notifications.
...
.. tab-set::
   ...
   .. tab-item:: Example
      ...
      assumes the Buckit deployment has at least one configured
...
The following table lists the supported :mc:`bm event add` values and their
