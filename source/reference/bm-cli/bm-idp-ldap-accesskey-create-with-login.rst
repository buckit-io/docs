...
===========================================
``bm idp ldap accesskey create-with-login``
===========================================
...
The :mc:`bm idp ldap accesskey create-with-login` uses interactive terminal-based prompt to authenticate with the external AD/LDAP server and generate access keys for use with Buckit.
...
.. tab-set::
   ...
   .. tab-item:: EXAMPLE
      ...
      .. code-block:: shell
         ...
         bm idp ldap accesskey create-with-login https://buckit.example.net/
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] idp ldap accesskey create-with-login        \
                                          URL                         \
                                          [--access-key <value>]      \
                                          [--secret-key <value>]      \
                                          [--policy <value>]          \
      ...
      - Replace ``URL`` with the :abbr:`FQDN (Fully Qualified Domain Name)` of a Buckit deployment configured for AD/LDAP integration.
...
.. mc-cmd:: URL
   ...
   The :abbr:`FQDN (Fully Qualified Domain Name)` of a Buckit deployment configured for AD/LDAP integration.
...
.. mc-cmd:: --access-key
   ...
   Omit to let Buckit randomly generate a value.
   
   The access key cannot contain the characters ``=`` (equal sign) or ``,`` (comma).

   Requires :mc-cmd:`~bm idp ldap accesskey create-with-login --secret-key`
...
.. mc-cmd:: --secret-key
   ...
   Requires :mc-cmd:`~bm idp ldap accesskey create-with-login --access-key`
...
.. mc-cmd:: --expiry-duration
   ...
   Mutually exclusive with :mc-cmd:`~bm idp ldap accesskey create-with-login --expiry`.
...
.. mc-cmd:: --expiry
   ...
   Mutually exclusive with :mc-cmd:`~bm idp ldap accesskey create-with-login --expiry-duration`.
...
The following command creates a new access key pair to use with the currently authenticated user on the Buckit deployment URL.
...
The following command creates a new access key pair with both an access key and secret key that you specify for the user currently authenticated on the Buckit deployment URL.

.. code-block:: shell
   :class: copyable

   bm idp ldap accesskey create-with-login https://buckit.example.net/ --access-key my-access-key-change-me --secret-key my-secret-key-change-me
...
.. code-block:: shell
   ...
   bm idp ldap accesskey create-with-login https://buckit.example.net --expiry-duration 24h
...
.. code-block:: shell
   ...
   bm idp ldap accesskey create-with-login https://buckit.example.net --expiry 2025-02-28
