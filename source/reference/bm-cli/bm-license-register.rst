...
.. mc:: bm support register
.. mc:: bm license register

.. important:: 

   ``bm license register`` requires :ref:`Buckit Manager CLI <minio-client>` version ``RELEASE.2023-11-20T16-30-59Z`` or later.
   While not strictly required, best practice keeps the :ref:`Buckit Manager CLI version <mc-client-versioning>` in alignment with the Buckit Server version.
...
The :mc-cmd:`bm license register` command connects your deployment with your |SUBNET| account.

.. end-mc-license-register-desc

After registration, you can upload deployment health reports directly to SUBNET using the :mc-cmd:`bm support diag` command.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] license register ALIAS                      \
                                  [--airgap]                          \
                                  [--api-key <string>]                \
...
.. mc-cmd:: --name
   ...
   Specify a name other than the alias to associate to the Buckit deployment in SUBNET.
...
Register the Buckit deployment at alias ``minio1`` on SUBNET, using ``minio1`` as the deployment name:

.. code-block:: shell
   :class: copyable

   bm license register minio1
...
Register a new Buckit deployment at alias ``minio5`` on SUBNET, using the license file downloaded for the account:

.. code-block:: shell
   :class: copyable

   bm license register minio5 /path/to/minio.license
...
Register a Buckit deployment at alias ``minio2`` on SUBNET, using ``second-deployment`` as the name:

.. code-block:: shell
   :class: copyable

   bm license register minio2 --name second-deployment
...
Register a Buckit deployment at alias ``minio3`` on SUBNET that does not have direct Internet access due to a firewall, airgap, or the like.

.. versionchanged:: mc RELEASE.2022-07-29T19-17-16Z

   The airgap registration process works with Buckit Manager CLI version ``RELEASE.2022-07-29T19-17-16Z`` or later.
   Earlier versions of the Buckit Manager CLI cannot register an airgapped deployment.

.. code-block:: shell
   :class: copyable

   bm license register minio3 --airgap
...
#. In the command line, run the following command
   ...
   .. code-block:: shell
      ...
      bm license update minio3 <path-to-file>
...
.. code-block:: shell
   ...
   bm [GLOBALFLAGS] license register       \
                            ALIAS          \
                            [--name value] \
                            [--airgap]
...
Once registered for |SUBNET|, Buckit automatically checks for and updates the license every month.

In airgapped or other environments where the server does not have direct access to the internet, use :mc-cmd:`bm license update` with the path to the file to update the registration.
