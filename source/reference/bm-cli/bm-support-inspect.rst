...
The :mc:`bm support inspect` command collects the data and metadata associated to objects at the specified path.

.. end-mc-support-inspect-desc

Buckit assembles this data from each backend drive storing an :ref:`erasure shard <minio-erasure-coding>` for each specified object.
The command produces an encrypted zip file that includes all matching files with their respective *host+drive+path*.

If this information is required to diagnose a |SUBNET| issue, Buckit Engineering will provide the appropriate command.
The resulting report is intended for use by Buckit Engineering via SUBNET and may contain internal or private data points associated to the object.
...
.. versionchanged:: RELEASE.2023-01-11T03-14-16Z
   ...
   The file uploads to Buckit for use by the engineering team in support efforts.
...
.. versionchanged:: RELEASE.2022-12-12T19-27-27Z
   ...
   When writing the zip archive, Buckit also encrypts the zip index of file names included in the archive.
...
.. important::
   ...
   :mc:`bm support inspect` requires a Buckit deployment server from October 2021 or later. 
...
.. code-block:: shell
   ...
   bm support inspect ALIAS/bucket/path/**/xl.meta
...
.. code-block:: shell
   ...
   bm support inspect minio1/mybucket/myobject/xl.meta
...
.. code-block:: shell
   ...
   bm support inspect minio1/mybucket/myobject/**
...
.. code-block:: shell
   ...
   bm [GLOBALFLAGS] support inspect       \
                            [--legacy]   \
                            TARGET
...
.. mc-cmd:: TARGET
   ...
   The path should include the `alias <alias>` of the Buckit deployment and, if needed, the prefix and/or object name.
