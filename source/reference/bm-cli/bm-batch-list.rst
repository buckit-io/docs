...
The :mc:`bm batch list` command outputs a list of the batch jobs currently in progress on a deployment.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] batch list TARGET           \
                                     --type "string"
...
- Replace ``replicate`` with the job type to output.
  ...
  Currently, :mc:`bm batch` only supports the ``replicate`` job type.
