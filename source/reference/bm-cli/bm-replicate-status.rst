...
The :mc:`bm replicate status` command displays the :ref:`replication status <minio-bucket-replication-serverside>` of a Buckit bucket.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] replicate status TARGET
                                    [--limit-upload value]
...
.. mc-cmd:: ALIAS
   ...
   The :ref:`alias <alias>` of the Buckit deployment and full path to the bucket or bucket prefix for which to display the replication status. 
   For example:
...
Use :mc:`bm replicate status` to show bucket replication status:

.. code-block:: shell
   :class: copyable

   bm replicate status ALIAS/PATH

- Replace :mc-cmd:`ALIAS <bm replicate status ALIAS>` with the :mc:`alias <bm alias>` of the Buckit deployment.

- Replace :mc-cmd:`PATH <bm replicate status ALIAS>` with the path to the bucket or bucket prefix.
