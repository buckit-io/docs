...
.. |command| replace:: :mc:`bm share download`
.. |versionid| replace:: :mc-cmd:`~bm share download --version-id`
.. |alias| replace:: :mc-cmd:`~bm share download ALIAS`
...
The :mc:`bm share download` command generates a temporary presigned URL with
integrated access credentials for downloading objects from a Buckit bucket. The
...
.. tab-set::
   ...
   .. tab-item:: EXAMPLE
      ...
      .. code-block:: shell
         ...
         bm share download --recursive mybuckit/mydata
...
.. mc-cmd:: ALIAS
   ...
   The :ref:`alias <alias>` of a Buckit deplyment and the full path to
   the object for which to generate a download URL. For example:

   .. code-block:: shell

      bm share download play/mybucket/object.txt
   ...
   .. code-block:: shell
      ...
      bm share download play/mybucket/object.txt play/mybucket/otherobject.txt

   If specifying the path to a bucket or bucket prefix, you **must** also
   specify the :mc-cmd:`~bm share download --recursive` argument. For
   example:

   .. code-block:: shell

      bm share download --recursive play/mybucket/

      bm share download --recursive play/mybucket/myprefix/
...
.. mc-cmd:: --recursive, r
   ...
   :mc-cmd:`bm share download ALIAS` bucket or bucket prefix. 
...
.. tab-set::
   ...
   .. tab-item:: Get Specific Object
      ...
      Use :mc:`bm share download` to generate a URL that supports
      ``GET`` requests for an object:

      .. code-block:: shell
         :class: copyable

         bm share download --expire DURATION ALIAS/PATH

      - Replace :mc-cmd:`ALIAS <bm share download ALIAS>` with the 
        :ref:`alias <alias>` of the Buckit deployment.

      - Replace :mc-cmd:`PATH <bm share download ALIAS>` with the path to the
        object on the Buckit deployment.

      - Replace :mc-cmd:`DURATION <bm share download --expire>` with the duration
        after which the URL expires. For example, to set a 30 day expiry, 
        specify ``30d``.

   .. tab-item:: Get Object(s) in a Bucket

      Use :mc:`bm share download` with the 
      :mc-cmd:`~bm share download --recursive` option to generate a URL for
      ...
      .. code-block:: shell
         ...
         bm share download --recursive --expire DURATION ALIAS/PATH

      - Replace :mc-cmd:`ALIAS <bm share download ALIAS>` with the 
        :ref:`alias <alias>` of the Buckit deployment.

      - Replace :mc-cmd:`PATH <bm share download ALIAS>` with the path to the
        bucket or bucket prefix on the Buckit deployment.
