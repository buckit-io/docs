...
The :mc:`bm undo` command reverses changes due to either a ``PUT`` or ``DELETE`` operation at a specified path.
...
.. mc-cmd:: --action
   ...
   By default, :mc:`bm undo` reverses both ``DELETE`` and ``PUT`` operations.
   Use :mc-cmd:`~bm undo --action` to choose one or the other, but only for the most recent operation of the specified type.
...
.. mc-cmd:: --last
   ...
   Mutually exclusive with :mc-cmd:`~bm undo --action`.
