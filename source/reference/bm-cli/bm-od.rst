...
The :mc:`bm od` command copies a local file to a remote location in a specified number of parts and part sizes.
The command outputs the time it took to upload the file. 

.. end-mc-od-desc

Use the :mc:`bm od` to mimic the functionality of the Linux ``dd`` command.
   ...
   .. tab-item:: SYNTAX
      ...
      .. code-block:: shell
         ...
         bm [GLOBALFLAGS] od                                            \
                          ...
                          of=<target Buckit path to upload to>           \
                          [size=<size of file>]                         \
...
.. mc-cmd:: size
   ...
   If not specified, Buckit determines the size for parts from the source stream.

.. mc-cmd:: parts
   :optional:

   The number of parts to divide the object into for uploading.
   If not specified, Buckit determines the number of parts based on the size of the source stream.
...
Use :mc:`bm od` to upload a file to Buckit in a set of parts of specified size. 
The :mc-cmd:`~bm od size` option allows you to specify the desired part size.
...
Buckit examines the source file and divides it into the necessary number of parts so that no part is larger than the specified 40MiB part size.

Upload a First Five 40 MiB Parts of a File
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc:`bm od` to upload parts of a file to Buckit of specified part size. 
The :mc-cmd:`~bm od size` option allows you to specify the desired part size.
The :mc-cmd:`~bm od parts` option allows you to specify the total number of parts to use for the object.
...
Take a source file, divide the file into a specified number of parts, then upload all parts of the file to a Buckit target.
