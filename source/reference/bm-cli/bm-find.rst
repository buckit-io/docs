...
The :mc:`bm find` command supports searching for objects on a Buckit deployment. 
...
.. mc-cmd:: ALIAS
   ...
   For objects on  Buckit or an S3-compatible host, specify the :ref:`alias <alias>` and the full path to search (e.g. bucket and prefixes). 
   For example:
   ...
   Issuing :mc-cmd:`bm find ALIAS` with no other arguments returns a list of
   *all* objects or files at the specified path, similar to :mc:`bm ls`.

.. mc-cmd:: --exec
   :optional:
   
   Spawns an external process for each object returned by :mc:`bm find`. 
...
.. mc-cmd:: --newer-than
   ...
   .. versionchanged:: RELEASE.2025-02-04T04-57-50Z
      ...
      For example, ``bm find --newer-than="2025-01-22 09:57:00 CET" minioalias/mybucket``.
...
.. mc-cmd:: --watch
   ...
   Continuously monitor the :mc-cmd:`~bm find ALIAS` and return any
...
- Replace :mc-cmd:`ALIAS <bm find ALIAS>` with the 
  :mc:`alias <bm alias>` of the S3-compatible host.

- Replace :mc-cmd:`PATH <bm find ALIAS>` with the path to a bucket on the
  S3-compatible host. Omit the path to search from the root of the S3 host.

- Replace :mc-cmd:`NAME <bm find --name>` with the object.
...
- Replace :mc-cmd:`EXTENSION <bm find --name>` with the file extension of the object.

Find All Matching Files and Copy To S3 Service
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc:`bm find` with the :mc-cmd:`~bm find --exec` option to find
files on a local filesystem and pass them to an :program:`bm` command for
further processing. The following example uses :mc:`bm cp` to copy the 
output of :mc:`bm find` to an S3-compatible host.
...
- Replace :mc-cmd:`FILEPATH <bm find ALIAS>` with the full file path to the
  directory to search.
...
include the :mc-cmd:`~bm find --watch` argument:
...
- Replace ``ALIAS`` with the :mc:`alias <bm alias>` of the Buckit deployment.
...
The :mc-cmd:`bm find --smaller` and :mc-cmd:`bm find --larger` flags
...
The :mc-cmd:`bm find --exec` and :mc-cmd:`bm find --print` commands
