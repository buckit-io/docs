...
.. |command| replace:: :mc:`bm retention info`
.. |rewind| replace:: :mc-cmd:`~bm retention info --rewind`
.. |versionid| replace:: :mc-cmd:`~bm retention info --version-id`
.. |alias| replace:: :mc-cmd:`~bm retention info ALIAS`
.. |versions| replace:: :mc-cmd:`~bm retention info --versions`
...
The :mc:`bm retention info` command configures the :ref:`Write-Once Read-Many (WORM)
...
:mc:`bm retention info` *requires* that the specified bucket has object locking
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      :mc-cmd:`bm retention info --version-id` is mutually exclusive with
...
.. mc-cmd:: ALIAS
   ...
   .. code-block:: shell
      ...
      bm retention info play/mybucket/object.txt

   - If the ``ALIAS`` specifies a bucket or bucket prefix, include 
      :mc-cmd:`~bm retention info --recursive` to return the object
      lock settings for all objects in the bucket or bucket prefix.

   - If the ``ALIAS`` bucket has versioning enabled, 
      :mc:`bm retention info` by default applies to only the latest object
      version. Use :mc-cmd:`~bm retention info --version-id` or
      :mc-cmd:`~bm retention info --versions` to return the object lock
...
.. mc-cmd:: --default
   ...
   to :mc-cmd:`~bm retention info ALIAS`.

   If specifying :mc-cmd:`~bm retention info --default`, 
   :mc:`bm retention info` ignores all other flags.

.. mc-cmd:: --recursive, r
   :optional:

   Recursively returns the object lock settings for all objects in the
   specified :mc-cmd:`~bm retention info ALIAS` path.

   Mutually exclusive with :mc-cmd:`~bm retention info --version-id`.
...
.. mc-cmd:: --version-id, vid
   ...
   - :mc-cmd:`~bm retention info --versions`
   - :mc-cmd:`~bm retention info --rewind`
   - :mc-cmd:`~bm retention info --recursive`
...
.. mc-cmd:: --versions
   ...
   Use :mc-cmd:`~bm retention info --versions` and
   :mc-cmd:`~bm retention info --rewind` together to retrieve the
...
.. tab-set::
   ...
   .. tab-item:: Specific Object
      ...
      .. code-block:: shell
         ...
         bm retention info ALIAS/PATH

      - Replace :mc-cmd:`ALIAS <bm retention info ALIAS>` with the
        :mc:`alias <bm alias>` of a configured S3-compatible host.

      - Replace :mc-cmd:`PATH <bm retention info ALIAS>` with the path to the
        object.

   .. tab-item:: Multiple Objects

      Use :mc:`bm retention info` with
      :mc-cmd:`~bm retention info --recursive` to retrieve the retention
      settings for all objects in a bucket:

      .. code-block:: shell
         :class: copyable

         bm retention info --recursive ALIAS/PATH
...
Use :mc:`bm retention info` with 
:mc-cmd:`~bm retention info --default` to retrieve the default 
object lock settings for a bucket:

.. code-block:: shell
   :class: copyable

   bm retention info --default ALIAS/PATH
