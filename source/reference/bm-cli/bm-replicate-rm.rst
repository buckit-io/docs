...
The :mc:`bm replicate rm` command removes a 
:ref:`replication rule <minio-bucket-replication-serverside>` from a 
Buckit bucket.

.. end-mc-replicate-rm-desc

The :mc:`bm replicate remove` command has equivalent functionality to :mc:`bm replicate rm`.

.. code-block:: shell

   bm [GLOBALFLAGS] replicate rm FLAGS [FLAGS] ALIAS
...
.. mc-cmd:: --id
   ...
   *Required* Specify the unique ID for a configured replication rule.
   ...
   You can omit this option if specifying :mc-cmd:`~bm replicate rm --all`

.. mc-cmd:: --all
   

   *Optional* Removes all replication rules on the specified bucket. Requires
   specifying the :mc-cmd:`~bm replicate rm --force` flag.

.. mc-cmd:: --force
   

   *Optional* Required if specifying :mc-cmd:`~bm replicate rm --all` .
...
Use :mc:`bm replicate rm` to remote a bucket replication rule:

.. code-block:: shell
   :class: copyable

   bm replicate rm --id "ID" ALIAS/PATH

- Replace :mc-cmd:`ID <bm replicate rm --id>` with the unique ID of the
  replication rule to remove. Use :mc:`bm replicate ls` to list all
  replication rules for the bucket.

- Replace :mc-cmd:`ALIAS <bm replicate rm ALIAS>` with the 
  :mc:`alias <bm alias>` of the Buckit deployment.

- Replace :mc-cmd:`PATH <bm replicate rm ALIAS>` with the path to the 
  bucket or bucket prefix.

Remove All Replication Rules from a Bucket
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc:`bm replicate rm` to list bucket replication rules:

.. code-block:: shell
   :class: copyable

   bm replicate rm --all --force ALIAS/PATH
...
Use The command or :mc:`bm rb` commands to remove replicated
