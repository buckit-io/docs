...
.. dropdown:: Command History
   ...
   .. list-table::
      ...
      * - Buckit Manager CLI Release
        - Command
        - Notes
   
      * - RELEASE.2022-02-13T23-26-13Z
        - ``bm support diag``
        - Command moved to ``bm support``
   
      * - RELEASE.2020-11-17T00-39-14Z 
        - ``bm admin subnet health``
      ...
      * - Original Command
        - ``bm admin obd``
        - Command renamed ``bm admin health``
...
The :mc-cmd:`bm support diag` command generates a health report for a Buckit deployment.

.. end-mc-support-diag-desc

For deployments registered with the Buckit subscription network (|subnet-short|), the command generates and uploads the health report for analysis.
Optionally, automate generating and uploading the report every 24 hours by enabling :mc-cmd:`~bm support callhome`.

The resulting health report is intended for use by Buckit Engineering via SUBNET and may contain internal or private data points.
Exercise caution before sending a health report to a third party or posting the health report in a public forum.

Buckit recommends that you run the health diagnostics when first provisioning the cluster and again at any failure scenario.

Use the :mc-cmd:`bm support diag` command to trigger the diagnostic test.
For clusters registered with SUBNET, the command uploads the results as part of SUBNET Health reports. 

For airgapped or firewalled environments, or other environments that prevent direct network access from the deployment, you can save the report locally with the :mc-cmd:`~bm support diag --airgap` flag.
...
.. code-block:: shell
   ...
   *********************************************************************************
   mc: Buckit diagnostics report saved to myminio-health_20231111053323.json.gz
...
Generate health data for a Buckit cluster and automatically for a Buckit cluster at alias ``minio1`` for transmission to SUBNET.

.. code-block:: shell
   :class: copyable

   bm support diag minio1
...
Generate a diagnostic report for a Buckit deployment at alias ``minio2`` and save it for manual upload to SUBNET:

.. code-block:: shell
   :class: copyable

   bm support diag minio2 --airgap
...
Generates health data for a Buckit cluster at alias ``myminio`` and anonymizes all sensitive data, including host names.

.. code-block:: shell
   :class: copyable

   bm support diag myminio --anonymize=strict
...
.. mc-cmd:: --anonymize
   ...
   Buckit anonymizes data loaded to SUBNET.
   Beginning with mc ``RELEASE.2023-11-10T21-37-17Z``, Buckit does *not* anonymize host names.
   ...
   .. code-block:: shell
      ...
      bm support diag minio --anonymize=strict
...
.. mc-cmd:: --api-key
   ...
   This value is only required for airgapped environments where Buckit has not already stored the API key for the deployment.
