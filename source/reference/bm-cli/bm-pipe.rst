...
The :mc:`bm pipe` command streams content from `STDIN <https://www.gnu.org/software/libc/manual/html_node/Standard-Streams.html>`__ to a target object.
...
.. tab-set::
   ...
   .. tab-item:: EXAMPLE
      ...
      .. code-block:: shell
         ...
         echo "My Meeting Notes" | bm pipe s3/engineering/meeting-notes.txt
...
.. mc-cmd:: --storage-class, --sc
   ...
   Set the storage class for the new object at the :mc-cmd:`~bm pipe TARGET`. 
...
.. code-block:: shell
   ...
   cat debian-live-11.5.0-amd64-mate.iso | bm pipe s3/opensource-isos/debian-11-5.iso

Stream MySQL Database Dump to S3
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The following command first streams a MySQL database and uses the stream to create a backup on S3 with :mc-cmd:`bm pipe`:

.. code-block:: shell
   :class: copyable

   mysqldump -u root -p ******* accountsdb | bm pipe s3/sql-backups/backups/accountsdb-sep-28-2022.sql
