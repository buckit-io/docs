...
:mc:`bm support profile` runs a system profile for your deployment.
The results of the profile can provide insight into the Buckit server process running on a given node.

.. end-mc-support-profile-desc

The resulting report is intended for use by Buckit Engineering.
...
This command retrieves the CPU profile on a Buckit deployment with the alias ``minio1``.
The profile runs for the default of 10 seconds.

.. code-block:: shell
   :class: copyable

   bm support profile --type cpu minio1/
...
.. code-block:: shell
   ...
   bm support profile --type cpu,mem,block minio2/
...
.. code-block:: shell
   ...
   bm support profile --type cpu,mem,block --duration 600 minio3/

Syntax
------
      
The :mc:`bm support profile` command has the following syntax:

.. code-block:: shell

   bm [GLOBALFLAGS] support profile       \
                            COMMAND       \
                            [--type]      \
                            [--airgap]    \
                            [--duration]  \
...
.. mc-cmd:: --type
   ...
   .. important::
      ...
      Do not use the ``cpuio`` or ``trace`` data types unless directed to by Buckit Support.
