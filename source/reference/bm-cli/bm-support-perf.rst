...
.. versionchanged:: RELEASE.2022-07-24T02-25-13Z
   ...
   ``bm support perf`` replaces the ``bm admin speedtest`` command.
...
Use the :mc:`bm support perf` command to review the performance of the S3 API (read/write), network IO, and storage (drive read/write).
...
For more complete performance testing, consider using a combination of load-testing using your staging application environments and the Buckit `WARP <https://github.com/minio/warp>`_ S3 benchmarking tool.

:mc:`bm support perf` has the following subcommands

#. :mc-cmd:`~bm support perf drive`

   Measure the speed of drives in a Buckit deployment.

   :mc-cmd:`bm support perf drive` temporarily suspends S3 API calls during the test.
   Incoming requests are held in a queue while the command runs.
   When the command completes or ends, Buckit processes the queued requests and resumes normal operations.

#. :mc-cmd:`~bm support perf object`
      
   Measure the speed of reading and writing objects in a cluster.

#. :mc-cmd:`~bm support perf net`

   Measure the network throughput of all nodes.

   :mc-cmd:`bm support perf net` temporarily suspends S3 API calls during the test.
   Incoming requests are held in a queue while the command runs.
   When the command completes or ends, Buckit processes the queued requests and resumes normal operations.

#. :mc-cmd:`~bm support perf client`

   Measure the network throughput to a client.

#. :mc-cmd:`~bm support perf site-replication`
...
Buckit autotunes concurrency to obtain maximum throughput and IOPS (Input/Output Per Second).

.. code-block:: shell
   :class: copyable
 
   bm support perf object minio1
...
Buckit autotunes concurrency to obtain maximum throughput.

.. code-block:: shell
   :class: copyable

   bm support perf object minio1 --duration 20s --size 128MiB
...
.. code-block:: shell
   ...
   bm support perf drive minio1 --blocksize 64KiB --filesize 2GiB
...
.. code-block:: shell
   ...
   bm support perf site-replication minio1
...
.. mc-cmd:: drive
   ...
   .. code-block:: shell
      ...
      bm [GLOBAL FLAGS] support perf drive   \
                      [--concurrent]         \
                      [--verbose, -v]        \
                      [--filesize]           \
                      [--blocksize]          \
...
.. mc-cmd:: object
   ...
   .. code-block:: shell
      ...
      bm [GLOBAL FLAGS] support perf object  \
                      [--size]               \
                      [--concurrent]         \
                      [--verbose, -v]        \
                      [--airgap]             \
...
.. mc-cmd:: net
   ...
   .. code-block:: shell
      ...
      bm [GLOBAL FLAGS] support perf net  \
                      [--concurrent]      \
                      [--verbose, -v]     \
                      [--serial]          \
                      [--airgap]          \
...
.. mc-cmd:: client
   ...
   Measure the network throughput from the local device running the Buckit Manager CLI to the server.

   .. code-block:: shell

      bm [GLOBAL FLAGS] support perf client  \
                      --duration             \
                      [--verbose, -v]        \
                      [--airgap]             \
                      ALIAS
...
.. mc-cmd:: site-replication
   ...
   .. code-block:: shell
      ...
      bm [GLOBAL FLAGS] support perf site-replication \
                        --duration                    \
                        [--verbose, -v]               \
                        ALIAS
...
.. mc-cmd:: --size
   ...
   Applies to the :mc-cmd:`~bm support perf object` command.
...
.. mc-cmd:: --concurrent
   ...
   Applies to the :mc-cmd:`~bm support perf drive`, :mc-cmd:`~bm support perf object`, and :mc-cmd:`~bm support perf net` commands.
...
.. mc-cmd:: --filesize
   ...
   Applies to the :mc-cmd:`~bm support perf drive` command.
...
.. mc-cmd:: --serial
   ...
   Applies to the :mc-cmd:`~bm support perf drive` and :mc-cmd:`~bm support perf net` commands.
...
.. mc-cmd:: ALIAS
   ...
   Applies to the :mc-cmd:`~bm support perf drive`, :mc-cmd:`~bm support perf object`, :mc-cmd:`~bm support perf net`, and :mc-cmd:`~bm support perf client` commands.
...
.. mc-cmd:: --duration
   ...
   Applies to the :mc-cmd:`~bm support perf client` command.
