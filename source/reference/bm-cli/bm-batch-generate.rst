...
The :mc:`bm batch generate` command creates a basic YAML-formatted template file for the specified job type.

.. end-mc-batch-generate-desc

After Buckit creates the file, open it in your preferred text editor tool to further customize.
...
.. code-block:: shell
   ...
   bm batch generate alias replicate > replicate.yaml

- Replace ``alias`` with the :mc:`alias <bm alias>` to use to generate the yaml file.

- Replace ``replicate`` with the type of job to generate a yaml file for.
 
  :mc:``bm batch`` supports the ``replicate`` and ``keyrotate`` job types. 
...
:mc:`bm batch` currently supports the following job task types:

- :ref:`minio-mc-batch-generate-replicate-job`
  
  Replicate objects between two Buckit deployments.
