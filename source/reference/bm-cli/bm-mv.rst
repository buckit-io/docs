...
The :mc:`bm mv` command moves an object from source to the target, such as
between Buckit deployments *or* between buckets on the same Buckit deployment.
:mc:`bm mv` also supports moving objects between a local filesystem and Buckit.

.. end-mc-mv-desc

You can also use :mc:`bm mv` against the local filesystem to produce
...
.. mc-cmd:: SOURCE
  ...
  The object or objects to move. 
   ...
   For moving an object from a Buckit bucket, specify the :ref:`alias <alias>`
   and the full path to the object(s) (e.g. bucket and path to objects). For
   example:

   .. code-block:: shell

      bm mv play/mybucket/object.txt play/myotherbucket/object.txt
   ...
   .. code-block:: shell
      ...
      bm mv ~/mydata/object.txt play/mybucket/object.txt
   
   Specify multiple ``SOURCE`` paths to move multiple objects to the
   specified :mc-cmd:`~bm mv TARGET`. :mc:`bm rm` treats the
   *last* specified alias or filesystem path as the ``TARGET``. For example:
   ...
   contents of that directory. If you omit the :mc-cmd:`~bm mv --recursive`
   argument, :mc:`~bm mv` only moves objects in the top level of the specified
   directory or bucket.
...
.. mc-cmd:: TARGET
   ...
   object(s) at the specified :mc-cmd:`~bm mv SOURCE`. Specify the 
   :ref:`alias <alias>` of a configured S3 service as the prefix to the 
   :mc-cmd:`~bm mv TARGET` path. 

   For moving an object from Buckit,
   ...
   If running :mc:`bm mv` with the :mc-cmd:`~bm mv --recursive` option, 
   :mc:`bm mv` treats the ``TARGET`` as the bucket prefix for all
...
.. mc-cmd:: --disable-multipart
   ...
   If any individual part upload fails, Buckit retries that part without affecting the other parts.
   After upload completes, the parts combine to restore the original object.

   Buckit recommends using multipart upload for any object larger than 100 MB.
...
.. mc-cmd:: --preserve, a
   ...
   :mc-cmd:`~bm mv SOURCE` directories, buckets, and objects on the 
   :mc-cmd:`~bm mv TARGET` bucket(s).

.. mc-cmd:: --recursive, r
   :optional:
   
   Recursively move the contents of each bucket or directory
   :mc-cmd:`~bm mv SOURCE` to the :mc-cmd:`~bm mv TARGET` bucket.
...
.. code-block:: shell
   ...
   bm mv [--recursive] FILEPATH ALIAS/PATH

- Replace :mc-cmd:`FILEPATH <bm mv SOURCE>` with the full file path to the
  file to move. 

  If specifying the path to a directory, include the 
  :mc-cmd:`~bm mv --recursive` flag.

  :mc:`bm mv` *removes* the files from the source after
  successfully moving it to the destination.

- Replace :mc-cmd:`ALIAS <bm mv TARGET>` with the :mc-cmd:`alias <bm alias>`
  of a configured S3-compatible host.

- Replace :mc-cmd:`PATH <bm mv TARGET>` with the destination bucket.

Move a File from Filesystem to S3-Compatible Host with Custom Metadata
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc:`bm mv` with the :mc-cmd:`~bm mv --attr` option to set custom
attributes on file(s).

.. code-block:: shell
   :class: copyable

   bm mv --attr "ATTRIBUTES" FILEPATH ALIAS/PATH

- Replace :mc-cmd:`FILEPATH <bm mv SOURCE>` with the full file path to the
  file to move. :mc:`bm mv` *removes* the file from the source after
  successfully moving it to the destination.
...
- Replace :mc-cmd:`ATTRIBUTES <bm mv --attr>` with one or more comma-separated
...
.. code-block:: shell
   :class: copyable
    ...
    bm mv --recursive SRCALIAS/SRCPATH TGTALIAS/TGTPATH

- Replace :mc-cmd:`SRCALIAS <bm mv SOURCE>` with the :mc-cmd:`alias <bm alias>`
  of a configured S3-compatible host.

- Replace :mc-cmd:`SRCPATH <bm mv SOURCE>` with the path to the bucket.
  :mc:`bm mv` *removes* the bucket and its contents from the source after
  successfully moving it to the destination.

- Replace :mc-cmd:`TGTALIAS <bm mv TARGET>` with the :mc-cmd:`alias <bm alias>`
  of a configured S3-compatible host.

- Replace :mc-cmd:`TGTPATH <bm mv TARGET>` with the path to the bucket.


Move File to S3-Compatible Host with Specific Storage Class
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :mc:`bm mv` with the :mc-cmd:`~bm mv --storage-class` option to set
the storage class on the destination S3-compatible host.

.. code-block:: shell
   :class: copyable

   bm mv --storage-class CLASS FILEPATH ALIAS/PATH

- Replace :mc-cmd:`CLASS <bm mv --storage-class>` with the storage class to 
...
- Replace :mc-cmd:`ATTRIBUTES <bm mv --attr>` with one or more comma-separated
  ...
  value.
   ...
   bm mv --storage-class REDUCED_REDUNDANCY myobject.txt play/mybucket
...
Buckit uses the :mc-cmd:`~bm mv SOURCE` object name when moving
the object to the :mc-cmd:`~bm mv TARGET` if no explicit target
object name is specified.

You can specify a different object name for the
:mc-cmd:`~bm mv TARGET` with the same object path to "rename"
an object. For example:

.. code-block:: shell

   bm mv play/mybucket/object.txt play/mybucket/myobject.txt

For recursive move operations (:mc-cmd:`bm mv --recursive`), Buckit
treats the ``TARGET`` path as a prefix for objects on the ``SOURCE``. 

Checksum Verification
~~~~~~~~~~~~~~~~~~~~~

:mc:`bm mv` verifies all move operations to object storage using MD5SUM
checksums. 
