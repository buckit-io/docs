...
The :mc:`bm ilm rule ls` command summarizes all configured object lifecycle management rules on a Buckit bucket in a tabular format.

.. end-mc-ilm-rule-ls-desc

The :mc:`bm ilm rule list` command has equivalent functionality to :mc:`bm ilm rule ls`.
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      The :mc:`bm ilm rule ls` command has the following syntax:

      .. code-block:: shell
         :class: copyable

         bm [GLOBALFLAGS] ilm rule ls     \
                          [--expiry]      \
                          [--transition]
...
.. mc-cmd:: ALIAS
   ...
   The :ref:`alias <alias>` and full path to the bucket on the Buckit deployment for which to list the object lifecycle management rules. 
   For example:
...
.. mc-cmd:: --expiry
   ...
   :mc:`bm ilm rule ls` returns only fields related to lifecycle rule expiration.

   Mutually exclusive with :mc-cmd:`~bm ilm rule ls --transition`.

.. mc-cmd:: --transition
   :optional:

   :mc:`bm ilm rule ls` returns only fields related to lifecycle rule transition.

   Mutually exclusive with :mc-cmd:`~bm ilm rule ls --expiry`.
...
Use :mc:`bm ilm rule ls` to list a bucket's lifecycle management rules:

.. code-block:: shell
   :class: copyable

   bm ilm rule ls ALIAS/PATH

- Replace :mc-cmd:`ALIAS <bm ilm rule ls ALIAS>` with the :mc:`alias <bm alias>` of the S3-compatible host.
...
Use :mc:`bm ilm rule ls` with :option::`--json <mc.--json>` to show the time the policy for a bucket was last updated.

.. code-block:: shell
   :class: copyable

   bm ilm rule ls ALIAS/PATH --json
