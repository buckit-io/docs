...
The :mc:`bm ilm tier add` command creates a new remote storage tier to a supported storage services.
...
:mc:`bm ilm tier add` supports *only* the following S3-compatible services as a remote target for object tiering:
...
Buckit requires the following administrative permissions on the cluster in which you create remote tiers for object transition lifecycle management rules:
...
.. tab-set::
   ...
   .. tab-item:: SYNTAX
      ...
      **^Note:** Each supported storage vendor authenticates with different methods.
      ...
      See details under :mc-cmd:`~bm ilm tier add TIER_TYPE` below. 
...
.. mc-cmd:: TIER_TYPE
   ...
   The Cloud Service Provider storage backend ("Tier") to which Buckit transitions objects. 
   ...
   .. list-table::
      ...
      * - ``minio``
        - Use a remote Buckit deployment as the storage backend for the new Tier.

          Requires also specifying the following parameters:

          - :mc-cmd:`~bm ilm tier add --access-key`
          - :mc-cmd:`~bm ilm tier add --secret-key`
      ...
      * - ``azure``
        - Use :abbr:`Azure (Microsoft Azure)` Blob Storage as the storage backend for the new Tier.
          ...
          - :mc-cmd:`~bm ilm tier add --account-name`
          - :mc-cmd:`~bm ilm tier add --account-key`
      ...
      * - ``gcs`` 
        - Use :abbr:`GCP (Google Cloud Platform)` Cloud Storage as the storage backend for the new Tier.
          ...
          - :mc-cmd:`~bm ilm tier add --credentials-file`

.. mc-cmd:: TARGET
   :required:

   The :mc-cmd:`alias <bm alias>` of a configured Buckit deployment on which the command creates the new remote tier.
   You can then create new rules with :mc:`bm ilm rule add` specifying the new remote tier.
      
.. mc-cmd:: TIER_NAME
   :required:

   The name to associate with the new remote tier. 
   The name **must** be unique across all configured tiers on the Buckit cluster.
...
.. mc-cmd:: --endpoint
   ...
   The URL endpoint for the S3 or Buckit storage. 
   The URL endpoint **must** resolve to the provider specified to :mc-cmd:`~bm ilm tier add TIER_TYPE`. 
...
.. mc-cmd:: --access-key
   ...
   Required if :mc-cmd:`~bm ilm tier add TIER_TYPE` is ``s3`` or ``minio``. 
   This option has no effect for any other value of ``TIER_TYPE``.
...
.. mc-cmd:: --account-name
   ...
   Required if :mc-cmd:`~bm ilm tier add TIER_TYPE` is ``azure``. 
   This option has no effect for any other value of ``TIER_TYPE``.

   Buckit does *not* support changing the storage account name associated to an Azure remote tier. 
...
.. mc-cmd:: --account-key
   ...
   The corresponding shared account key for the :mc-cmd:`~bm ilm tier add --account-name` associated to the remote Azure tier.
...
.. mc-cmd:: --credentials-file
   ...
   Required if :mc-cmd:`~bm ilm tier add TIER_TYPE` is ``gcs``. 
   This option has no effect for any other value of ``TIER_TYPE``.
...
.. mc-cmd:: --prefix
   ...
   The prefix path for the specified :mc-cmd:`~bm ilm tier add --bucket` to which Buckit transitions objects.
...
.. mc-cmd:: --storage-class
   ...
   The storage class ("access tier" for Microsoft Azure) Buckit applies to objects transitioned to the remote bucket.

   The storage class to apply to objects transitioned by Buckit to the remote bucket.
...
.. mc-cmd:: --region
   ...
   The S3 backend region for the specified :mc-cmd:`~bm ilm tier add TIER_TYPE`, such as ``us-west-1``.
...
.. mc-cmd:: --aws-role-arn
   ...
   This option only applies if :mc-cmd:`~bm ilm tier add TIER_TYPE` is ``s3`` **and** the source is a Buckit pod on Amazon EKS.
...
.. mc-cmd:: --azure-sp-tenant-id
   ...
   This option only applies if :mc-cmd:`~bm ilm tier add TIER_TYPE` is ``azure`` and you log in using a service principal identity.
   This option has no effect for any other value of ``TIER_TYPE``.
