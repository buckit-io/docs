...
Specify the name of the Kafka topic to which Buckit publishes bucket events.
...
Buckit supports the following mechanisms:
...
Buckit stores undelivered events in the specified store while the Kafka server/broker is offline and replays the stored events when connectivity resumes.
